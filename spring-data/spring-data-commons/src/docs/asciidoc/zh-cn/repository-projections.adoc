...
NOTE: 在 `Repository` 中声明一个覆盖基本方法的方法（例如，在 `CrudRepository`、特定于存储的存储库接口或 `Simple...Repository` 中声明）会导致调用基本方法，而不管声明的内容如何返回类型。
确保使用兼容的返回类型，因为基本方法不能用于投影。 一些商店模块支持 `@Query` 注解以将覆盖的基本方法转换为查询方法，然后可用于返回投影。
...
NOTE: 检查 `Class`  类型的查询参数是否符合动态投影参数的条件。 如果查询的实际返回类型等于 `Class` 参数的通用参数类型，则匹配的 `Class` 参数不可用于查询或 SpEL 表达式中。
如果您想使用 `Class` 参数作为查询参数，请确保使用不同的通用参数，例如 `Class<?>`。