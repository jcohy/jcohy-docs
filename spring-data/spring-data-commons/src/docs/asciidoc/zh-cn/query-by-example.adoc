...
* `FetchableFluentQuery`: `FetchableFluentQuery` 提供了一个流式 API，允许进一步自定义从 `Example` 派生的查询。
使用流式的 API，您可以为查询指定排序投影和结果处理。
...
NOTE: 由于可空性， 可以将属性包含在 "按示例查询" . 除非 <<query-by-example.matchers,`ExampleMatcher` 忽略属性路径>>， 否则始终包括使用基本类型(`int`, `double`, …) 的属性 .
