...
NOTE: keyspace notification 消息监听器会更改 Redis 中的 `notify-keyspace-events` 设置(如果尚未设置). 现有设置不会被覆盖,因此您必须正确设置这些设置(或将其保留为空). 请注意,在 AWS `ElastiCache` 上禁用了 `CONFIG`,并且启用监听器会导致错误.
