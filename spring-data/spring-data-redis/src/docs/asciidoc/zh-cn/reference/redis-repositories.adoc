...
我们在这里有一个非常简单的域对象. 请注意,它的类型上带有 `@RedisHash` 注解,并带有 `org.springframework.data.annotation.Id` 注解的名为 `id` 的属性. 这两个选项负责创建实际的 key.
...
NOTE: keyspace notification 消息监听器会更改Redis中的 `notify-keyspace-events` 设置(如果尚未设置). 现有设置不会被覆盖,因此您必须正确设置这些设置(或将其保留为空). 请注意,在AWS `ElastiCache` 上禁用了 `CONFIG`,并且启用监听器会导致错误.
