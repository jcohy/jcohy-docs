...
`Jackson2HashMapper` 通过使用 https://github.com/FasterXML/jackson[FasterXML Jackson] 为域对象提供 Redis 哈希映射. `Jackson2HashMapper` 可以将顶级属性映射为 Hash 字段名称,并且可以选择将结构展平. 简单类型映射到简单值. 复杂类型(嵌套对象,集合,地图等)表示为嵌套 JSON.
