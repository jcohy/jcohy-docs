...
`com.mongodb.client.MongoClient` 对象是由 `MongoClientFactoryBean` 在 `@Configuration` 注解的类中创建的，要访问 `com.mongodb.client.MongoClient` ，请使用 `private @Autowired MongoClient mongoClient;` 字段。
