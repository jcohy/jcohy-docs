...
默认情况下,  Spring Data REST 暴露 HTTP 资源和方法,  如 <<repository-resources>> 中所述基于存储库 CRUD 方法暴露.  该存储库无需扩展 `CrudRepository`,  但也可以有选择地声明上述部分中描述的方法,  随后将暴露资源.  例如.  如果存储库未暴露 `delete(…)` 方法,  则项目资源将不支持 HTTP DELETE.
