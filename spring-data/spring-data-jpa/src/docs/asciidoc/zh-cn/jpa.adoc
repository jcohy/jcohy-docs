...
使用 `repositories`  元素可按  "`<<repositories.adoc#repositories.create-instances,创建存储库实例>>`" 中所述查找 Spring Data 存储库.  除此之外,它还为所有使用 `@Repository` 注解的 bean 激活持久性异常转换,以将 JPA 持久性导致的异常转换为 Spring 的 `DataAccessException` 层次结构.
