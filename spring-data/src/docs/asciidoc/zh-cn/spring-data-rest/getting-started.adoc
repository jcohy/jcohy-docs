...
要将 Spring Data REST 添加到基于 Gradle 的项目中，请将 spring-data-rest-webmvc artifact 添加到您的编译时依赖项中，如下所示：
...
要将 Spring Data REST 添加到基于 Maven 的项目中，请将 `spring-data-rest-webmvc` artifact 添加到编译时依赖项中：
...
要自定义配置，请注册 `RepositoryRestConfigurer`（或扩展 `RepositoryRestConfigurerAdapter`）的实现或覆盖与您的用例相关的  `configure…` -方法。
...
Spring Data REST 使用 `RepositoryDetectionStrategy` 来确定是否将存储库导出为REST资源。 `RepositoryDiscoveryStrategies` 枚举包括以下值：

.Repository 检测策略
[cols="1,5". options="header"]
|===
| 枚举值         | 描述
| `DEFAULT`    | 公开所有公共存储库接口， 除了 `@(Repository)RestResource` 的 `exported` 标志。
| `ALL`        | 暴露所有独立可见的类型和注解的 repositories
| `ANNOTATION` | 只有使用 `@(Repository)RestResource`  注解的存储库才公开，除非其 `exported` 标志设置为 `false`。
...
在 Spring Boot 1.2 和更高版本中，您可以通过在 `application.properties` 中设置单个属性来更改基本URI，如下所示：
...
另外，您可以注册一个自定义实现 `RepositoryRestConfigurer` 作为Spring bean，并确保它被组件扫描拾取，如下所示：
...
这些指南介绍了如何为相关数据存储添加依赖项，配置 domain 对象以及定义存储库。
...
it should work with any Spring Data project that supports the repository programming model. The data stores listed above are the ones for which we have written integration tests to verify that Spring Data REST works with them.
