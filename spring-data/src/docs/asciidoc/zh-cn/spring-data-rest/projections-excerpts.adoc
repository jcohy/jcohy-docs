...
Spring Data REST 暴露了  <<metadata.alps>>  文档,这是一种微元数据格式. 请按照根资源暴露的  `profile` 链接. 如果您向下导航至 `Person` 资源 (即 `/alps/persons`) 的ALPS文档,则可以找到有关 `Person` 资源的许多详细信息. 将列出投影以及关于 `GET` REST 转换的详细信息,如下所示:
