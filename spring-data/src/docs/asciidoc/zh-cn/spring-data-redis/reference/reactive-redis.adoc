...
`ReactiveRedisConnectionFactory` 创建活动的 `ReactiveRedisConnection` 实例. 另外,工厂还充当 `PersistenceExceptionTranslator` 实例,这意味着工厂一旦声明,就可以进行透明的异常转换-例如,通过使用 `@Repository` 批注和AOP进行异常转换.
