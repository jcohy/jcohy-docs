...
+
打开第二个“文件资源管理器”窗口，然后转到下载 Gradle 发行版的目录。双击ZIP存档以显示内容。将内容文件夹 `gradle-6.7` 拖到新创建的 `C:\Gradle` 文件夹中。
+
...
+
在 "系统变量" 下，选择 `Path`，然后单击 `编辑` 。为 `C:\Gradle\gradle-6.7\bin` 添加一个条目。单击确定保存。
+
