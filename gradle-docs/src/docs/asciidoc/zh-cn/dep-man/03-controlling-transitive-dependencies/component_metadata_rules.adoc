...
从存储库中提取的每个模块都有与之关联的元数据,例如其 group,name,version 以及它提供的带有 artifacts 和依赖的不同变体. 有时,此元数据不完整或不正确. 为了从构建脚本中处理这种不完整的元数据,Gradle 提供了一个 API 来编写组件元数据规则. 这些规则在下载模块的元数据之后但在依赖关系解析中使用之前生效.
...
组件元数据规则 API 面向 link:https://github.com/gradle/gradle/blob/master/subprojects/docs/src/docs/design/gradle-module-metadata-latest-specification.md[Gradle Module Metadata] 和构建脚本中的 dependencies API 支持的功能. 编写规则与在构建脚本中定义依赖和 artifacts 之间的主要区别在于,组件元数据规则遵循 Gradle Module 元数据的结构,直接对 <<variant_model.adoc#understanding-variant-selection,variants>> 进行操作. 相反,在构建脚本中,您经常一次影响多个变体类型 (例如,将 api 依赖添加到 Java 库的 api 和运行时变体中,由 jar 任务生成的 artifacts 也添加到这两个变体中)  .
...
根据模块元数据的格式,它以不同的方式映射到元数据的以变体为中心的表示形式:

- 如果模块具有 Gradle 模块元数据,则规则所依据的数据结构与您在模块的 `.module` 文件中找到的数据结构非常相似.
- 如果仅使用 `.pom` 元数据发布该模块,则会扩展 许多固定的变体,如 <<variant_model.adoc#sub:maven-mapping-to-variants,mapping of POM files to variants>>  部分中所述.
...
如果模块是使用传统的元数据发布的 (仅 `.pom` 或 `ivy.xml`,没有 `.module` 文件) ,则元数据很可能不完整,因为这些格式不支持诸如变体或依赖约束之类的功能. 仍然,从概念上讲,此类模块可以包含不同的变体,或者可能具有它们刚刚被忽略 (或错误地定义为依赖) 的依赖约束. 在下一部分中,我们将探讨许多现有的 oss 模块,这些模块具有不完整的元数据以及添加缺少的元数据信息的规则.
...
如果仔细研究 Jaxen 1.1.4 pom,我们会发现  _dom4j_, _jdom_ and _xerces_ 赖项仍然存在,但标记为可选. poms 中的可选依赖不会被 Gradle 或 Maven 自动处理.原因是它们表明 Jaxen 库提供了一些可选的 <<feature_variants.adoc#,optional feature variants>>,它们需要一个或多个这样的依赖,但是缺少了这些特性是什么以及哪些依赖属于哪些的信息.这样的信息不能在 pom 文件中表示,而是在 Gradle 模块元数据中通过变体和 <<component_capabilities.adoc#declaring-component-capabilities,capabilities>> 表示.因此,
...
这样,所有其他 Java 生态系统属性都已经正确设置,并且所有依赖都被继承. 然后,将两个变体的 `TARGET_JVM_VERSION_ATTRIBUTE` 都设置为 `8`,使用 `removeAllFiles()` 从新变体中删除任何现有文件,
并使用 `addFile()` 添加 jdk8 jar 文件. 需要 `removeAllFiles()`,因为从相应的基本变体中复制了对主jar `quasar-core-0.7.5.jar` 的引用.

我们还利用针对 Java 7 的信息 `attribute(TARGET_JVM_VERSION_ATTRIBUTE, 7)` 丰富了现有的编译和运行时变体.

现在,我们可以为构建脚本中的所有依赖于编译类路径的依赖请求 Java 8 版本,并且 Gradle 会自动为每个库选择最合适的变体. 在 Quasar 的情况下,它将是 jdk8Compile 变体,
...
例如,可以使用它来定义运行时和编译变体.相应规则的示例可以在 <<variant_model.adoc#sub:ivy-mapping-to-variants,here>> 找到.Ivy 配置的 Ivy 细节(例如依赖和文件)也可以使用 `withVariant(configurationName)`  API修改.
...
Gradle 和 Gradle 模块元数据还允许在整个组件上设置属性,而不是单个变体. 这些属性中的每一个都具有特殊的语义,因为它们影响版本选择,而版本选择是在变体选择之前完成的.
