...
从存储库中提取的每个模块都有与之关联的元数据,例如其 group,name,version 以及它提供的带有 artifacts 和依赖的不同变量. 有时,此元数据不完整或不正确. 为了从构建脚本中处理这种不完整的元数据,Gradle 提供了一个 API 来编写组件元数据规则. 这些规则在下载模块的元数据之后但在依赖关系解析中使用之前生效.

== 编写组件元数据规则的基础

组件元数据规则应用于构建脚本或 settings 脚本的依赖关系块 (link:{groovyDslPath}/org.gradle.api.artifacts.dsl.DependencyHandler.html[DependencyHandler]) 的组件 (link:{groovyDslPath}/org.gradle.api.artifacts.dsl.ComponentMetadataHandler.html[ComponentMetadataHandler]) 部分中.可以用两种不同的方式定义规则:

1. 直接在组件部分中应用它们时作为操作
2. 实现 link:{javadocPath}/org/gradle/api/artifacts/ComponentMetadataRule.html[ComponentMetadataRule] 接口

虽然将规则内联定义为操作可以方便地进行实验,但是通常建议将规则定义为单独的类. 可以使用 `@CacheableRule` 注解作为隔离类编写的规则,以缓存其应用程序的结果,从而使每次解决依赖关系时都无需重新执行它们.
...
另外,可以将 Gradle 服务注入到 `ComponentMetadataRule` 中. 因此,一旦有了构造函数,就必须使用 `@javax.inject.Inject` 对其进行注解. 通常需要使用 link:{javadocPath}/org/gradle/api/model/ObjectFactory.html[ObjectFactory] 服务来创建强类型值对象 (例如用于设置 link:{javadocPath}/org/gradle/api/attributes/Attribute.html[Attribute] 的值) 的实例. link:{javadocPath}/org/gradle/api/artifacts/repositories/RepositoryResourceAccessor.html[RepositoryResourceAccessor] 是有助于组件元数据规则与自定义元数据的高级使用的服务.
...
组件元数据规则 API 面向 link:https://github.com/gradle/gradle/blob/master/subprojects/docs/src/docs/design/gradle-module-metadata-latest-specification.md[Gradle Module Metadata] 和构建脚本中的 dependencies API 支持的功能. 编写规则与在构建脚本中定义依赖和 artifacts 之间的主要区别在于,组件元数据规则遵循 Gradle Module 元数据的结构,直接对 <<variant_model.adoc#understanding-variant-selection,variants>> 进行操作. 相反,在构建脚本中,您经常一次影响多个变量的形状 (例如,将 api 依赖添加到 Java 库的 api 和运行时变量中,由 jar 任务生成的 artifacts 也添加到这两个变量中)  .
...
- 如果仅使用 `.pom` 元数据发布该模块,则会扩展 许多固定的变量,如 <<variant_model.adoc#sub:maven-mapping-to-variants,mapping of POM files to variants>>  部分中所述.
- 如果仅使用 `ivy.xml` 文件发布该模块,则可以访问文件中定义的 Ivy 配置,而不是 variant. 它们的依赖性,依赖性约束和文件可以修改
  另外,如果需要,可以使用 `addVariant(name, baseVariantOrConfiguration) { }` API 从 Ivy 配置扩展  variant (例如,可以使用此方法定义 <<variant_model.adoc#sub:ivy-mapping-to-variants,_compile_ and _runtime_ variants for the Java library plugin>>) .

== 何时使用组件元数据规则?

通常,如果您考虑使用组件元数据规则来调整某个模块的元数据,则应首先检查该模块是使用 Gradle 模块元数据 (`.module` 文件) 还是仅使用传统元数据 (`.pom` or `ivy.xml`) 发布的.

如果使用 Gradle Module Metadata 发布了模块,则尽管有时仍然存在某些明显错误的情况,但元数据可能已完成. 对于这些模块,只有在明确确定元数据本身存在问题的情况下,才应使用组件元数据规则. 如果您对依赖解析结果有疑问,则应首先通过 <<rich_versions.adoc#,dependency constraints with rich versions>>. 来检查是否可以解决问题. 特别是,如果您正在开发要发布的库,则应记住,与组件元数据规则相反,依赖关系约束是作为您自己的库的元数据的一部分发布的. 因此,在具有依赖关系约束的情况下,您可以自动与使用者共享依赖关系解决方案的解决方案,而组件元数据规则仅应用于自己的内部版本.

如果模块是使用传统的元数据发布的 (仅 `.pom` 或 `ivy.xml`,没有 `.module` 文件) ,则元数据很可能不完整,因为这些格式不支持诸如变量或依赖约束之类的功能. 仍然,从概念上讲,此类模块可以包含不同的变量,或者可能具有它们刚刚被忽略 (或错误地定义为依赖) 的依赖约束. 在下一部分中,我们将探讨许多现有的 oss 模块,这些模块具有不完整的元数据以及添加缺少的元数据信息的规则.
...
如果仔细研究 Jaxen 1.1.4 pom,我们会发现  _dom4j_, _jdom_ and _xerces_ 赖项仍然存在,但标记为可选. poms 中的可选依赖不会被 Gradle 或 Maven 自动处理.原因是它们表明 Jaxen 库提供了一些可选的 <<feature_variants.adoc#,optional feature variants>>,它们需要一个或多个这样的依赖,但是缺少了这些特性是什么以及哪些依赖属于哪些的信息.这样的信息不能在 pom 文件中表示,而是在 Gradle 模块元数据中通过变量和 <<component_capabilities.adoc#declaring-component-capabilities,capabilities>> 表示.因此,
...
在这里,我们首先使用 `addVariant(name, baseVariant)` 方法创建一个额外的变量,通过定义一个新功能 _jaxen-dom4j_  来表示 Jaxen 的可选 dom4j 集成特性,我们将其标识为特性变量.这类似于在构建脚本中 <<feature_variants.adoc#sec::declare_feature_variants,defining optional feature variants>> .

然后我们使用一个 `add` 方法来添加依赖,以定义这个可选特性需要哪些依赖.在构建脚本中,我们可以向可选特性添加一个依赖,Gradle 将使用丰富的元数据来发现正确的传递依赖
...
在上一个示例中,所有变量 ("main variants" 和可选功能) 都打包在一个 jar 文件中,但通常会将某些变量发布为单独的文件. 特别是,当变量是互斥的时,即它们不是功能变量,而是提供替代选择的不同变量. 所有基于 pom 的库都已经存在的一个示例是运行时和编译变量,其中 Gradle 只能根据手头的任务选择一个. 在 Java 生态系统中经常发现的此类替代方法中的另一个是针对不同 Java 版本的 jar.

例如,我们看一下在 link:https://repo1.maven.org/maven2/co/paralleluniverse/quasar-core/0.7.9[Maven central] 上发布的异步编程库 Quasar 的 0.7.9版 本. 如果检查目录列表,则会发现除了 `quasar-core-0.7.9.jar` 之外,还发布了 `quasar-core-0.7.9-jdk8.jar`. 使用 _classifier_ (此处为 jdk8) 发布其他 jar 是 maven 存储库中的常见做法. 尽管 Maven 和 Gradle 都允许您通过分类器引用此类 jar,但它们在元数据中根本没有提及. 因此,不存在这些 jar 存在的信息,以及此类 jar 表示的变量之间是否存在其他差异 (如不同的依赖) .

在 Gradle 模块元数据中,将显示此变量信息,对于已经发布的 Quasar 库,我们可以使用以下规则添加它:
...
在这种情况下,很明显,classifier 代表目标 Java 版本,这是已知的 <<variant_model.adoc#sec:variant-aware-matching,Java 生态系统属性>> . 因为我们还需要 Java 8 的编译和运行时,所以我们创建了两个新的变量,但将现有的编译和运行时变量用作基础.
这样,所有其他 Java 生态系统属性都已经正确设置,并且所有依赖都被继承. 然后,将两个变量的 `TARGET_JVM_VERSION_ATTRIBUTE` 都设置为 `8`,使用 `removeAllFiles()` 从新变量中删除任何现有文件,
并使用 `addFile()` 添加 jdk8 jar 文件. 需要 `removeAllFiles()`,因为从相应的基本变量中复制了对主jar `quasar-core-0.7.5.jar` 的引用.

我们还利用针对 Java 7 的信息 `attribute(TARGET_JVM_VERSION_ATTRIBUTE, 7)` 丰富了现有的编译和运行时变量.

现在,我们可以为构建脚本中的所有依赖于编译类路径的依赖请求 Java 8 版本,并且 Gradle 会自动为每个库选择最合适的变量. 在 Quasar 的情况下,它将是 jdk8Compile 变量,
...
为同一个库发布多个替代方案的另一种解决方案是使用流行的 Guava 库所做的版本控制模式. 在这里,通过将分类器 (而不是 jar  artifacts ) 附加到版本,每个新版本都会发布两次. 以 Guava 28 为例,
...
带有 classifiers 的 Jars 还用于将库中存在多个替代方案 (例如本地代码) 与主要 artifacts 分离的部分. 例如,这是由轻量级 Java 游戏库 (LWGJ) 完成的,该库将一些特定于平台的 jar 发布到 link:https://repo1.maven.org/maven2/org/lwjgl/lwjgl/3.2.3[Maven central],
在运行时,除了主 jar 之外,始终需要其中的一个. 由于没有通过元数据关联多个 artifacts 的概念,因此无法在 pom 元数据中传达此信息. 在 Gradle 模块元数据中,每个变体可以具有任意多个文件,
我们可以通过编写以下规则来利用它:
...
所有 Java 生态系统属性,依赖和主 jar 文件都属于每个运行时变体的一部分. 我们仅设置其他属性 `OPERATING_SYSTEM_ATTRIBUTE` 和 `ARCHITECTURE_ATTRIBUTE`,它们被定义为 Gradle <<building_cpp_projects.adoc#,native support>> 的一部分.
然后,我们添加相应的本地 jar 文件,以使每个运行时变体现在都包含两个文件: 主 jar 和本地 jar.
...
而是选择一种预先组合的变体 (在一个 jar 中提供所有内容) . 这样的库之一就是在 link:https://repo1.maven.org/maven2/com/google/inject/guice/4.2.2[Maven central] 上发布的著名的依赖注入框架 Guice,
...
例如,可以使用它来定义运行时和编译变量.相应规则的示例可以在 <<variant_model.adoc#sub:ivy-mapping-to-variants,here>> 找到.Ivy 配置的 Ivy 细节(例如依赖和文件)也可以使用 `withVariant(configurationName)`  API修改.
