...
有关如何使用配置，检查和后处理分配的依赖的元数据和工件的更多示例，请查看 <<dependency_resolution.adoc#sec:programmatic_api,resolution result APIs>>。
...
该配置从 `testImplementation` 配置扩展以重用现有的测试框架依赖关系。
...
配置是 Gradle 中依赖关系解析的基本部分。在依赖关系解析的上下文中，区分消费者和生产者非常有用。按照这些原则，配置至少具有3个不同的角色：

1. 声明依赖
2. 作为消费者，以解决文件的一系列依赖关系
3. 作为生产者，将工件及其依赖项公开以供其他项目使用（此类消耗性配置通常表示生产者向其消费者提供的  <<variant_model.adoc#,variants>>）
...
配置可以通过扩展从其他配置继承依赖关系。 现在，请注意，上面的代码没有告诉我们有关此配置的目标使用者的任何信息。 特别是，它并没有告诉我们如何使用配置。 假设 `lib` 是一个 Java 库：
...
- `someConfiguration` 声明我的应用程序的依赖关系。这只是一个可以存储依赖项列表的存储桶。
- `compileClasspath` 和 `runtimeClasspath` 是要解决的配置：解决后，它们应分别包含编译类路径和应用程序的运行时类路径。

这种区别由 `Configuration` 类型中的 `canBeResolved` 标志表示。 可以解析的配置是我们可以为其计算依赖关系图的配置，因为它包含实现解析所需的所有必要信息。
也就是说，我们将计算一个依赖图，解析图中的组件，并最终获得工件。 `canBeResolved` 设置为 `false` 的配置并不意味着可以解决。 这样的配置仅用于声明依赖项。
...
另一方面，在 library 项目方面（生产者），我们还使用配置来表示可以消耗的东西。 例如，该库可能会公开一个 API 或 runtime，并且我们会将工件附加到一个或多个。
通常，要针对 `lib` 进行编译，我们需要 `lib` 的API，但不需要其运行时依赖项。 因此，`lib` 项目将公开一个 `apiElements` 配置，该配置面向正在寻找其API的消费者。
...
声明依赖的配置的选择很重要。 但是，没有固定的规则必须将依赖项放入哪个配置中。 它主要取决于配置的组织方式，这通常是所应用插件的属性。

例如，在 `java` 插件中，<<java_plugin.adoc#tab:configurations,documented>> 创建的配置，并应根据其在代码中的作用，将其用作确定在哪里声明依赖项的基础。
...
假设您要声明对 link:https://tomcat.apache.org/tomcat-9.0-doc/jasper-howto.html[Jasper Ant task] 的依赖关系，以预编译 JSP 文件，这些文件不应最终出现在用于编译源代码的类路径中。
...
以了解有关 API 的全部信息。 该 API 提供了属性和配置方法。 通过字符串符号，您可以定义属性的子集。 使用地图符号可以定义所有属性。 要使用 map 或字符串表示法访问完整的API，可以将单个依赖项与闭包一起分配给配置。

[NOTE]
====
如果声明模块依赖性，Gradle将在存储库中查找模块元数据文件 (`.module`, `.pom` or `ivy.xml`)。 如果存在这样的模块元数据文件，将对其进行解析，并下载该模块的构件 (e.g. `hibernate-3.0.5.jar`) 及其依赖项 (e.g. `cglib`)。
如果不存在这样的模块元数据文件，那么从 Gradle 6.0 开始，您需要  <<declaring_repositories.adoc#sec:supported_metadata_sources,配置元数据源定义>> 以直接查找名为 `hibernate-3.0.5.jar` 的工件文件。
====
...
在 Gradle 和 Ivy 中，一个模块可以具有多个工件。 每个工件可以具有不同的依赖关系集。
====

[[sub:file_dependencies]]
=== 文件依赖

项目有时不依赖于二进制存储库产品，例如 JFrog Artifactory 或 Sonatype Nexus，用于托管和解决外部依赖关系。 通常的做法是将这些依赖项托管在共享驱动器上，或者将其与项目源代码一起放入版本控制中。
这些依赖项称为文件依赖项，原因是它们表示文件没有附加任何  <<dependency_management_terminology.adoc#sub:terminology_module_metadata,元数据>>（例如有关传递性依赖项，源或其作者的信息）。
...
以下示例从 `ant`, `libs` 和 `tools` 目录解析文件依赖关系。
...
在代码示例中可以看到，每个依赖项都必须定义其在文件系统中的确切位置。 创建文件引用的最主要方法是 link:{groovyDslPath}/org.gradle.api.Project.html#org.gradle.api.Project:files(java.lang.Object++[]++)[Project.files(java.lang.Object...)],
...
或者，您可以 还以 <<declaring_repositories.adoc#sub:flat_dir_resolver,flat directory repository>>平面目录存储库的形式定义一个或多个文件依赖项的源目录。

[NOTE]
====
即使在单台计算机上，`FileTree` 中文件的顺序也不稳定。 这意味着以这种构造作为种子的依赖项配置可能会产生具有不同顺序的解析结果，可能会影响使用结果作为输入的任务的可缓存性。 建议尽可能使用较简单的文件。
====

文件依赖性使您可以直接将一组文件添加到配置中，而无需先将它们添加到存储库中。 如果您不能或不想将某些文件放在存储库中，这将很有用。 或者，如果您根本不想使用任何存储库来存储依赖项。

要将某些文件添加为配置的依赖项，只需将 <<working_with_files.adoc#sec:file_collections,file collection>> 作为依赖项传递：
...
Gradle 可以对模块之间的依赖关系进行建模。这些依赖关系称为  _项目依赖_ ，因为每个模块均由Gradle项目表示。
...
在运行时，该构建会自动确保以正确的顺序构建项目依赖项，并将其添加到类路径中以进行编译。 <<multi_project_builds.adoc#multi_project_builds,Authoring Multi-Project Builds>> 一章讨论了如何更详细地设置和配置多项目构建。

有关更多信息，请参见 link:{javadocPath}/org/gradle/api/artifacts/ProjectDependency.html[ProjectDependency] 的 API 文档。
 
以下示例从 `web-service` 项目声明对 `utils` 和 `api` 项目的依赖关系。 方法 link:{groovyDslPath}/org.gradle.api.Project.html#org.gradle.api.Project:project(java.lang.String)[Project.project(java.lang.String)] 通过路径创建对特定子项目的引用。
...
您可以使用 link:{groovyDslPath}/org.gradle.api.artifacts.dsl.DependencyHandler.html#org.gradle.api.artifacts.dsl.DependencyHandler:gradleApi()[DependencyHandler.gradleApi()] 方法声明对当前版本的Gradle API的依赖关系。在开发自定义 Gradle 任务或插件时，这很有用。
...
您可以使用 link:{groovyDslPath}/org.gradle.api.artifacts.dsl.DependencyHandler.html#org.gradle.api.artifacts.dsl.DependencyHandler:gradleTestKit()[DependencyHandler.gradleTestKit()] 方法声明对当前版本的Gradle的TestKit API的依赖关系。这对于编写和执行Gradle插件和构建脚本的功能测试很有用。
...
您可以使用 link:{groovyDslPath}/org.gradle.api.artifacts.dsl.DependencyHandler.html#org.gradle.api.artifacts.dsl.DependencyHandler:localGroovy()[DependencyHandler.localGroovy()] 方法声明与 Gradle 一起分发的 Groovy 的依赖关系。在 Groovy 中开发自定义 Gradle 任务或插件时，这很有用。
...
这使构建脚本中的依赖项声明和 <<viewing_debugging_dependencies.adoc#sec:identifying_reason_dependency_selection,依赖报告>> 更易于理解。
...
每当 Gradle 尝试从 Maven 或 Ivy 存储库解析模块时，它都会查找元数据文件和默认工件文件 JAR。 如果这些工件文件都无法解析，则构建将失败。
在某些情况下，您可能需要调整 Gradle 解决依赖关系工件的方式。

- 依赖项仅提供非标准工件，而没有任何元数据，例如 一个ZIP文件。
- 模块元数据声明了多个工件，例如 作为常春藤依赖项描述符的一部分。
- 您只希望下载特定的工件，而无需在元数据中声明任何传递依赖项。

Gradle 是一个多语言构建工具，不仅限于解析 Java 库。 假设您想使用 JavaScript 作为客户端技术来构建Web应用程序。 大多数项目将外部 JavaScript 库检入版本控制。
外部 JavaScript 库与可重用 Java 库没有什么不同，那么为什么不从存储库中下载它呢？

link:https://developers.google.com/speed/libraries/[Google Hosted Libraries] 是一个流行的开源 JavaScript 库的分发平台。 借助仅工件的符号，您可以下载 JavaScript 库文件，例如 jQuery 的。 `@` 字符将依赖项的坐标与工件的文件扩展名分开。
...
某些模块提供了相同工件的不同“风味”，或者它们发布了属于特定模块版本但目的不同的多个工件。 Java库通常会使用已编译的类文件来发布工件，另一个库中只包含源代码，而第三个库中包含Javadocs。

在 JavaScript 中，库可能以未压缩或缩小的工件形式存在。 在 Gradle 中，特定的工件标识符称为 classifier ，该术语通常在 Maven 和 Ivy 依赖管理中使用。

假设我们要下载 JQuery 库的精简构件，而不是未压缩的文件。 您可以提供 classifier `min` 作为依赖项声明的一部分。
...
外部模块依赖项需要模块元数据（因此，Gradle通常可以确定模块的可传递依赖项）。 为此，Gradle支持不同的元数据格式。
