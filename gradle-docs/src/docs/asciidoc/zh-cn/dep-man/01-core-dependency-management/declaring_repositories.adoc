...
Gradle 可以从一个或多个基于 Maven、Ivy 或目录格式的仓库中解决依赖.<<#sec:repository-types,查看所有类型的仓库的完整参考>>资料以获得更多信息.

[[sec:declaring_public_repository]]
== 声明一个公开可用的仓库

构建软件的组织可能希望利用公共二进制仓库来下载和使用开源依赖. 流行的公共仓库包括 <<#sub:maven_central,Maven Central>> and the <<#sub:maven_google,Google Android>> 仓库. Gradle 为这些广泛使用的仓库提供了内置的速记符号.

.Declaring a repository with the help of shorthand notations
image::{image-dir}/dependency-management-shortcut-repositories.png[]
...
Maven Central 是一个流行的仓库,托管开放源代码库供 Java 项目使用.
...
Google 仓库托管特定于 Android 的 artifacts ,包括 Android SDK. 有关使用示例,请参阅 link:https://developer.android.com/studio/build/dependencies.html#google-maven[相关的 Android 文档].
...
* 不同格式
** 一个与 <<#sec:maven_repo,Maven 兼容>> 的 artifact 仓库(e.g: Maven Central, ...)
...
一些项目可能更喜欢将依赖存储在共享驱动器上,或者作为项目源代码的一部分而不是二进制仓库产品来存储.如果要将 (平面) 文件系统目录用作仓库,只需输入:
...
此外,还有按解析上下文过滤的选项: 配置名称甚至配置属性.有关详细信息,请参见 link:{javadocPath}/org/gradle/api/artifacts/repositories/RepositoryContentDescriptor.html[RepositoryContentDescriptor].

=== 声明仅在一个仓库中找到的内容

不包含使用<<#sec:declaring-repository-filter,repository-level content filter>> 声明的过滤器
...
Gradle 提供了一种在所有项目的中心位置声明它们的方法，而不是在构建的每个子项目中或通过 `allprojects` 块声明存储库。

NOTE: 集中声明存储库的是一个孵化功能

每个子项目按惯例使用的存储库可以在 `settings.gradle(.kts)` 文件中声明:

.Declaring a Maven repository in settings
====
include::{snippets-dir}/artifacts/defineRepositoryInSettings/groovy/settings.gradle[tags=declare_repositories_settings]
include::{snippets-dir}/artifacts/defineRepositoryInSettings/kotlin/settings.gradle.kts[tags=declare_repositories_settings]
====

`dependencyResolutionManagement` 存储库块接受与项目中相同的符号，其中包括 Maven 或 Ivy 存储库，有或没有凭据等。

默认情况下，项目声明的存储库将 *覆盖* settings 中声明的任何内容。 您可以更改此行为以确保始终使用 settings 存储库：

.Preferring settings repositories
====
include::{snippets-dir}/artifacts/defineRepositoryInSettings/groovy/settings.gradle[tags=prefer_settings]
include::{snippets-dir}/artifacts/defineRepositoryInSettings/kotlin/settings.gradle.kts[tags=prefer_settings]
====

[[sub:fail_build_on_project_repositories]]

如果出于某种原因，项目或插件在项目中声明了存储库，Gradle 会警告您。
但是，如果您想强制仅使用 settings 存储库，则可以使其 _fail the build_：

.Enforcing settings repositories
====
include::{snippets-dir}/artifacts/defineRepositoryInSettings/groovy/settings.gradle[tags=enforce_settings]
include::{snippets-dir}/artifacts/defineRepositoryInSettings/kotlin/settings.gradle.kts[tags=enforce_settings]
====

最终，默认相当于调用  `preferProjectRepositories`:

.Preferring project repositories
====
include::{snippets-dir}/artifacts/defineRepositoryInSettings/groovy/settings.gradle[tags=prefer_projects]
include::{snippets-dir}/artifacts/defineRepositoryInSettings/kotlin/settings.gradle.kts[tags=prefer_projects]
====
...
Gradle 的默认行为是仅在服务器以 HTTP 401 响应的形式响应身份验证质询时提交凭据. 在某些情况下,服务器将以不同的代码响应 (例如,对于托管在 GitHub 上的仓库,返回 404) ,
...
指定在 S3 服务器 HTTP 5xx 状态代码响应的情况下重试请求的最大次数。 如果未指定，则使用默认值 3。

[[sub:s3_url_formats]]
=== S3 URL formats

S3 的 URL 格式是 'virtual-hosted-style' ，必须采用以下格式
...
* `/maven/release` 是 AWS S3 Key（存储桶中对象的唯一标识符）
...
如果 `org.gradle.s3.endpoint` 属性使用 HTTP（不是 HTTPS）URI 指定，则可以使用以下系统代理设置：
...
一些 AWS S3 区域 (eu-central-1 - Frankfurt) 要求所有 HTTP 请求都按照 AWS 的 http://docs.aws.amazon.com/general/latest/gr/signature-version-4.html[signature version 4] 进行签名。 建议在使用需要 V4 签名的存储桶时指定包含特定区域端点的 S3 URL。 例如
...
[NOTE]
====
如果没有为需要 V4 签名的存储桶指定特定区域的端点，Gradle 将使用默认的 AWS 区域 (us-east-1) ，
控制台上会出现以下警告:
...
以下系统属性可用于配置与 link:https://cloud.google.com/storage/[Google Cloud Storage] 存储库交互
...
Google Cloud Storage URL 是 'virtual-hosted-style' ，必须采用以下格式 `gcs://&lt;bucketName&gt;/&lt;objectKey&gt;`

e.g. `gcs://myBucket/maven/release`

* `myBucket` Google Cloud Storage bucket 名.
* `/maven/release` Google Cloud Storage Key（存储桶中对象的唯一标识符）
...
请注意,配置属性前缀-身份-是从仓库名称确定的. 然后可以以任何受支持的方式为 Gradle Properties 提供凭据- `gradle.properties` 文件,命令行参数,环境变体或这些选项的组合.
