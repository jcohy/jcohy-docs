...
生成的锁定文件将在其名称前加上 `buildscript-` 前缀，以防止与常规项目配置的锁定发生任何名称冲突。

=== Locking settings classpath configuration

通过在 <<build_lifecycle.adoc#sec:settings_file,Gradle settings 文件>> 中执行与上述相同的操作，您可以锁定 settings `classpath` 配置或您在设置中解析的任何自定义配置。 生成的锁定文件将在其名称前添加一个 `settings-` 前缀，以防止与项目配置或项目构建脚本的任何名称冲突。
