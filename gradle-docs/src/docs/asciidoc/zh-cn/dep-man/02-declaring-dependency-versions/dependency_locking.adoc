...
使用动态依赖项版本 (e.g. `1.+` or `[1.0,2.0)`) 会使构建不确定。 这会导致构建中断而没有任何明显的变化，更糟糕的是，可能是由构建作者无法控制的传递依赖导致的。

为了实现  https://reproducible-builds.org/[reproducible builds]，必须锁定依赖项和传递性依赖项的版本，以便具有相同输入的构建将始终解析相同的模块版本。 这称为依赖锁定。
...
即使生成了新版本，也会导致选择的依赖项相同。 对构建的修改会影响已解决的依赖关系集，将导致其失败。 这样可以确保发布的依赖项或构建定义中的更改不会在不调整锁定状态的情况下更改分辨率。

[NOTE]
====
依赖关系锁定仅对于 <<dynamic_versions.adoc#sub:declaring_dependency_with_dynamic_version,dynamic versions>> 才有意义。 尽管内容可能会更改，但它对坐标保持不变的 <<dynamic_versions.adoc#sub:declaring_dependency_with_changing_version,changing versions>>（例如 `-SNAPSHOT`）没有影响。 
...
Strict mode::
在这种模式下，除了上述验证之外，如果标记为locked的配置没有与之关联的锁定状态，则依赖项锁定将失败。
...
一旦激活了功能预览（见上文），您就可以简单地按照文档来 <<#lock_all_configurations_in_one_build_execution, writing>> 或  <<#selectively_updating_lock_state_entries, updating>> 依赖项锁定状态。
...
JVM 生态系统中的一个简单示例是 Scala 版本，该版本通常在工件坐标中找到。
...
在重现性不是主要目标的情况下，可以使用依赖项锁定。 作为构建作者，您可能希望依赖版本更新的频率有所不同，例如，取决于它们的来源。 在这种情况下，忽略某些依赖项可能很方便，因为您始终想使用最新版本。
...
该表示法是 `<group>:<name>` 依赖项表示法，其中 `\*`  可用作尾随通配符。 有关更多详细信息，请参见有关更新锁定文件的 <<#selectively_updating_lock_state_entries,the description>>。 请注意，不接受值 `*:*`，因为它等效于禁用锁定。
