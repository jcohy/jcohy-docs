...
消费者需要解释它需要什么类型变体，这是通过在 _consumer_ 上设置 _attributes_ 来完成的。
...
作为消费者，我们可以通过附加 `(org.gradle.usage,JAVA_API)` 属性来表达我们需要可解析配置的依赖项的 API。 这样做，Gradle 可以通过查看配置属性来_自动选择适当的变体_：
