[[upgrading_version_5]]
= 从 Gradle 5.x 更新到 6.0

本章提供了将 Gradle 5.x 构建迁移到 Gradle 6.0 所需的信息。 要从 Gradle 4.x 迁移，请先完成 <<upgrading_version_4.adoc#upgrading_version_4, 4.x 到 5.0 指南>>。

我们建议所有用户执行以下步骤：

. 尝试运行 `gradle help --scan` 并在扫面的结果中查看 https://gradle.com/enterprise/releases/2018.4/#identify-usages-of-deprecated-gradle-functionality[deprecations view]。
+
image::{image-dir}/deprecations.png[Deprecations View of a Gradle Build Scan]
+
这样您就可以看到您的构建中的任何弃用警告。
+
或者，您可以通过运行 `gradle help --warning-mode=all` 在控制台中查看弃用，尽管它可能不会报告那么多详细信息。
. 更新你的插件。
+
一些插件将与这个新版本的 Gradle 不兼容，例如因为它们使用已删除或更改的内部 API。 当插件尝试使用的 API 已弃用时，上一步将通过发出弃用警告来帮助您识别潜在问题。
+
. 运行 `gradle wrapper --gradle-version {gradleVersion}` 将项目更新至 {gradleVersion}.
. 尝试运行项目和调试任务错误使用 <<troubleshooting.adoc#troubleshooting, Troubleshooting 指南>>.
