...
Gradle 和 Maven 对如何构建项目有根本不同的看法。 Gradle 提供了一种灵活且可扩展的构建模型，该模型将实际工作委托给 <<what_is_gradle#the_core_model_is_based_on_tasks,任务依赖关系图>>。 Maven 使用固定的线性阶段模型，可以在其中附加目标（完成工作的事物）。
...
TIP: 保留旧的 Maven 构建和新的 Gradle 构建。 您知道 Maven 构建可以工作，因此您应该保留它，直到确信 Gradle 构建可以生成所有相同的工件，然后再做您需要的事情。 这也意味着用户无需获取复制源码即可尝试 Gradle 构建。
...
+
请注意，Gradle 构建不需要您 "install" 工件，因为您可以使用更合适的功能，例如 <<declaring_dependencies.adoc#sub:project_dependencies,项目间依赖关系>> 和 <<composite_builds#,组合构建>> 。 您只应使用 `publishToMavenLocal` 与 `Maven` 构建进行互操作。
+
Gradle 还允许您解决对本地 Maven 缓存的依赖关系，如 <<migmvn:declaring_repos,声明仓库>>部分中所述。
...
Gradle 的依赖关系管理系统比 Maven 的依赖项管理系统更灵活，但是它仍然支持相同的存储库，声明的依赖关系，作用域（Gradle 中的 <<declaring_dependencies.adoc#sec:what-are-dependency-configurations,依赖关系配置>>）和可传递依赖关系的概念。
...
Gradle 使用与 Maven 相同的依赖标识符组件：group ID, artifact ID 和 version.。 它还支持 classifiers。 因此，您所需要做的就是将标识符的依赖信息替换为 Gradle 的语法，这在 <<declaring_dependencies#,声明依赖项>> 一章中进行了介绍。
...
`test`::
Gradle 区分了 _compile_  项目测试所需的那些依赖项和仅  _run_  它们所需的那些依赖项。
+
...
使用 `compileOnly` 配置。
+
请注意，<<war_plugin#sec:war_dependency_management,War 插件>>添加了 `provideCompile` 和 `providerRuntime` 依赖项配置。 它们的行为与 `compileOnly` 略有不同，只需确保 WAR 文件中没有打包这些依赖项即可。
...
+
您还可以指定对 POM-only 的发布的常规依赖性。 在这种情况下，在该 POM 中声明的依赖关系将被视为构建的常规传递依赖关系。
+
...
====
+
这样做的结果是，将所有 `groovy-POM` 中的所有  `compile` 和 `runtime` 作用域依赖项都添加到测试运行时类路径中，而仅将 `compile` 作用域依赖添加到测试编译类路径中。 与其他作用域的依赖关系将被忽略。
...
最后，Gradle 允许您解决对 <<declaring_repositories.adoc#sub:maven_local,本地Maven缓存/存储库>>的依赖关系。 这有助于 Gradle 构建与 Maven 构建进行互操作，但是如果您不需要这种互操作性，则不应该使用该技术。
如果要通过文件系统共享已发布的工件，请考虑使用 `file://` URL 配置 <<declaring_repositories.adoc#sec:maven_repo,自定义 Maven 存储库>>。
...
如果要确保多项目构建中所有项目的版本一致性，类似于 Maven 中的 `<dependencyManagement>` 块的工作方式，则可以使用 <<java_platform_plugin#java_platform_plugin,Java Platform Plugin>>。 这允许您声明一组可以应用于多个项目的依赖项约束。
...
如果您出于与版本无关的原因要排除依赖项，请查看 <<dependency_downgrade_and_exclude.adoc#sec:excluding-transitive-deps,排除依赖传递>> 中的部分。 它显示了如何将排除项附加到整个配置（通常是最合适的解决方案）或依赖项。 您甚至可以轻松地将排除应用于所有配置。
...
至于将依赖项发布为可选内容，Gradle 提供了一个更丰富的模型，称为 <<feature_variants#feature_variants,功能变量>>，该模型可以让您声明库提供的 "可选功能"。
...
NOTE: 您可以使用此功能将来自任何依赖项的 POM 的 `<dependencyManagement>` 信息应用于 Gradle 构建，即使那些没有 `pom` 打包类型的信息也是如此。 `platform()` 和 `forcedPlatform()` 都将忽略在 `<dependencies>` 块中声明的所有依赖项。
...
----
 2. 用 <<declaring_dependencies.adoc#sub:project_dependencies,项目依赖>> 替换跨模块依赖项.
...
Checkstyle 任务会自动添加为 `check` 任务的依赖项，其中也包括 `test`。 如果要确保Checkstyle在测试之前运行，则只需使用 mustRunAfter() 方法指定顺序：
...
值得记住的是，Gradle 版本通常比 Maven 版本更易于扩展和自定义。 在这种情况下，这意味着您可能不需要 Gradle 插件来替换 Maven。 例如，Maven Enforcer 插件允许您控制依赖项版本和环境因素，
