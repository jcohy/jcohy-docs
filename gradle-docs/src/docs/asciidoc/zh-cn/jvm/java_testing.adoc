...
在 JVM 上的测试是一个丰富的主题。有许多不同的测试库和框架，以及许多不同类型的测试。无论它们执行的是否频繁,都需要包含在 gradle 构建之中。本章致力于解释 Gradle 如何处理不同版本之间和内部的不同需求，并重点介绍了它如何与两个最常见的测试框架: https://junit.org/[JUnit] 和 https://testng.org/[TestNG]。
