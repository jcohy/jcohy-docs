 ...
 * 源文件及其位置
 * 编译类路径，包括任何必需的依赖项（通过 Gradle <<dependency_management_terminology.adoc#sub:terminology_configuration,配置>>）
 * 编译后的类文件输出位置

您可以在此图中看到它们之间的相互关系:

.Source sets and Java compilation
image::images/java-sourcesets-compilation.png[]
...
image::images/java-sourcesets-process-resources.png[]
...
许多 Java 项目都使用源文件以外的资源，例如图像，配置文件和本地数据。 有时，这些文件只需要原封不动地打包，有时需要将它们作为模板文件或以其他方式进行处理。
...
除了在  _src/test/java_ 中提供单元测试的自动编译功能之外，Java 库插件还对运行使用 JUnit 3、4 和 5 的测试提供了本机支持（ https://docs.gradle.org/4.6/release-notes.html#junit-5-support[ Gradle 4.6] 中提供了对 JUnit 5 的支持）和 TestNG。 你得到：
...
如果依赖项的类型出现在库的公共类的公共字段或方法中，则该依赖项将通过库的公共 API 公开，因此应将其添加到 `api` 配置中。 否则，依赖项是内部实现细节，应将其添加到 _implementation_ 中。
...
无法将打包为 JAR 的 Java 应用程序设置为易于从命令行或桌面环境中启动。 <<application_plugin.adoc#application_plugin,Application Plugin> 通过创建一个分发来解决命令行方面的问题，该分发包括生产 JAR，其依赖项以及类似 Unix 和 Windows 系统的启动脚本。
 ...
 * 将库依赖项复制到 WAR 的 _WEB-INF/lib_ 子目录中
...
Java Platforms 代表了一组依赖项声明和约束，这些声明和约束形成了要在消费项目上应用的内聚单元。 该平台没有来源，也没有自己的工件。 它在 Maven 世界中映射到 https://maven.apache.org/guides/introduction/introduction-to-dependency-mechanism.html#Dependency_Management[BOM]。
