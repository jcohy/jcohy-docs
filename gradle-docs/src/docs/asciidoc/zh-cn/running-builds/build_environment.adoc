...
* <<#sec:gradle_system_properties, System properties（系统属性）>> 例如存储在 `gradle.properties` 文件中的 `systemProp.http.proxyHost=somehost.org`.
* <<#sec:gradle_configuration_properties, Gradle properties（Gradle 属性）>> 例如 `org.gradle.caching=true`,通常存储在项目根目录或 `GRADLE_USER_HOME` 环境变量的 `gradle.properties` 文件中.
* <<#sec:gradle_environment_variables, Environment variables（环境变量）>> 例如执行 Gradle 的环境来源的 `GRADLE_OPTS`.

除了配置构建环境之外,还可以使用 <<#sec:project_properties, Project properties（项目属性）>>(例如 `-PreleaseType=final`)配置给定的项目构建.

[[sec:gradle_configuration_properties]]
== Gradle 属性

Gradle 提供了几个选项,可以轻松配置将用于执行构建的 Java 流程.虽然可以通过 `GRADLE_OPTS` 或 `JAVA_OPTS` 在本地环境中配置这些设置,但将某些设置(例如 JVM 内存配置和 Java 主目录位置(`JAVA_HOME`))存储在版本控制中很有用,这样整个团队就可以在一致的环境中工作.
...
设置为 `true` 时,单个输入属性哈希值和每个任务的构建缓存键都记录在控制台上.了解有关 <<build_cache.adoc#sec:task_output_caching,输出缓存任务>>的更多信息.

`org.gradle.configureondemand=(true,false)`::
启用 <<multi_project_configuration_and_execution.adoc#sec:configuration_on_demand, configuration on demand(按需配置)>>,Gradle 将尝试仅配置必要的项目.
...
指定用于 Gradle Daemon 的 JVM 参数.该设置对于 <<<<#sec:configuring_jvm_memory,configuring JVM memory settings>>,配置 JVM 内存>>设置以提高构建性能特别有用.这不会影响 Gradle 客户端 VM 的 JVM 设置.
...
配置后,Gradle 将扩展 到 `org.gradle.workers.max` ,以并行执行项目.要了解有关并行任务执行的更多信息,请参阅 <<performance.adoc#parallel_execution,Gradle 构建性能章节>>..
...
指定 Gradle 用户的主目录.
`https.protocols`::
以逗号分隔格式指定支持的 TLS 版本. 例如: `TLSv1.2,TLSv1.3`.
...
当 Gradle 看到特别命名的系统属性或环境变量时,它也可以设置项目属性.如果环境变量名称看起来像 `ORG_GRADLE_PROJECT___prop__=somevalue`,则 Gradle 将在项目对象上设置 `prop` 属性,其值为``somevalue`. Gradle也为系统属性支持此功能,但是具有不同的命名模式,类似于 `org.gradle.project.__prop__`.以下两个都将 Project 对象上的 `foo` 属性设置为 `"bar"`.
...
[NOTE]
====
如果引用了 Project 属性但该属性不存在,则将引发异常,并且构建将失败.
...
请参阅 link:{groovyDslPath}/org.gradle.api.tasks.testing.Test.html[Test] API 文档中的其他示例,以及 <<java_testing.adoc#sec:test_execution, Java 插件参考中的测试执行>>.
...
.使用 `gradle.properties` 配置 HTTPS 代理
----
...
您可能需要设置其他属性才能访问其他网络.这里有 2 个参考可能会有所帮助:
...
如果您的代理服务器需要 NTLM 身份验证,则可能需要提供身份验证域以及用户名和密码.您可以通过两种方式提供用于向 NTLM 代理进行身份验证的域:
