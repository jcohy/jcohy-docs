...
====
+
然而，在一个区域中，将构建脚本视为可执行代码很有用：了解构建脚本的语法如何映射到 Gradle 的API。由 https://docs.gradle.org/6.7/dsl/[Groovy DSL Reference] 和 https://docs.gradle.org/6.6.1/javadoc/[Javadocs] 组成的 API 文档列出了方法和属性，并引用了闭包和操作。这些在构建脚本的上下文中是什么意思？查看 <<reference-groovy>>，以了解该问题的答案，以便您可以有效地使用API​​文档。
+
[TIP]
====
