...
大多数构建工作需要操作文件,Gradle 增加了一些 API 帮助您处理这些工作.
...
您甚至可以直接使用路径而不使用 `file()` 方法,如在 <<#sec:copying_files,文件深度复制>> 部分中前面所述:
...
现在考虑另一个示例: 如果您想复制目录中的所有PDF而不必指定每个 PDF,该怎么办?  为此,请将包含和/或排除模式附加到副本规范中. 在这里,我们使用字符串模式仅包含PDF:
...
image::{image-dir}/copy-with-flat-filter-example.png[]
...
image::{image-dir}/copy-with-deep-filter-example.png[]
...
从 Gradle 的角度来看,将文件打包到 archive 中实际上是一个副本,其中目标是 archive 文件而不是文件系统上的目录. 这意味着创建存档看起来很像具有相同功能的复制！
...
archives 实际上是独立的文件系统,因此解压缩它们是将文件从该文件系统复制到本地文件系统-甚至复制到另一个 archive 的情况. \ Gradle通过提供一些包装器功能来实现此目的,这些包装器功能使 archive 可以作为文件 ((<<sec:file_trees,file trees>>)) 的分层集合使用.
...
可以通过 link:{groovyDslPath}/org.gradle.api.tasks.AbstractCopyTask.html#eachFile(org.gradle.api.Action)[eachFile()] 方法处理更高级的处理. 例如,您可能需要将存档的不同子树提取到目标目录内的不同路径中. 以下示例使用该方法将存档的  `libs`  目录中的文件提取到根目标目录中,而不是 `libs` 子目录中:
...
为了对文件执行某些操作,您需要知道它在哪里,这就是文件路径所提供的信息. Gradle 建立在标准 `{javaApi}/java/io/File.html[File]` 类的基础上,该类表示单个文件的位置,并提供了用于处理路径集合的新 API. 本节说明如何使用 Gradle API 指定用于任务和文件操作的文件路径.
...
文件集合只是由 FileCollection 接口表示的一组文件路径. 任何文件路径. 请务必理解,文件路径不必以任何方式关联,因此它们不必位于同一目录中,甚至不必具有共享的父目录. 您还将发现 Gradle API 的许多部分都使用 FileCollection,例如本章稍后讨论的复制API和依赖配置.
...
多部分都使用 FileCollection,例如本章稍后讨论的复制 API 中的<<declaring_dependencies.adoc#sec:what-are-dependency-configurations,dependency configurations>>.
...
建议的指定文件集合的方法是使用 `ProjectLayout.files (java.lang.Object ...)` 方法,该方法返回 `FileCollection` 实例. 此方法非常灵活,可让您传递多个字符串,文件实例,字符串集合,文件集合等.
...
文件树就是一个按照层次结构分布的文件集合,例如,一个文件树可以代表一个目录树结构或者一个 ZIP 压缩文件的内容.并且具有 link:{javadocPath}/org/gradle/api/file/FileTree.html[FileTree] 类型. 这意味着文件树中的所有路径必须具有相同的父目录.下图突出显示了在复制文件的常见情况下文件树和文件集合之间的区别:

.The differences in how file trees and file collections behave when copying files
image::{image-dir}/file-collection-vs-file-tree.png[]
...
归档是打包到单个文件中的目录和文件层次结构. 换句话说,这是文件树的特例,这正是 Gradle 处理 archive 的方式. 可以使用 link:{groovyDslPath}/org.gradle.api.Project.html#org.gradle.api.Project:zipTree(java.lang.Object)[Project.zipTree(java.lang.Object)] 和 link:{groovyDslPath}/org.gradle.api.Project.html#org.gradle.api.Project:tarTree(java.lang.Object)[Project.tarTree(java.lang.Object)] 方法来包装相应类型的存档文件,
而不是使用仅适用于普通文件系统的 `fileTree()` 方法.  (请注意,JAR,WAR 和 EAR 文件是 ZIP) . 两种方法都返回 `FileTree`  实例,然后您可以使用它们与普通文件树相同的方式使用它们.
例如,您可以通过将归档文件的内容复制到文件系统上的某个目录中来提取归档文件的部分或全部文件. 或者,您可以将一个 archive 合并到另一个 archive 中.
...
image::{image-dir}/exploded-war-child-copy-spec-example.png[]

这不是简单的副本,因为项目中不存在  `WEB-INF` 目录及其子目录,因此必须在复制过程中创建它们. 另外,我们只希望 HTML 和 images 文件直接进入根文件夹 ( `build/explodedWar`) ,而仅 JavaScript 文件进入 js 目录.
...
link:{groovyDslPath}/org.gradle.api.tasks.Sync.html[Sync] 任务扩展了复制任务,将源文件复制到目标目录中,然后从目标目录中删除所有未复制的文件. 换句话说,它将目录的内容与其源同步. 这对于执行诸如安装应用程序,创建 archive 的分解副本或维护项目依赖的副本等操作很有用.
...
archives 本质上是独立的文件系统,Gradle 就这样对待它们. 这就是为什么使用存档与使用文件和目录非常相似,包括文件权限之类的原因.
...
创建 archive 文件本质上是一个文件副本,其中目标是隐式的,即 archive 文件本身. 这是一个基本示例,用于指定目标存档文件的路径和名称:
...
在下一部分中,您将了解基于约定的 archive 名称,这可以避免您始终配置目标目录和 archive 名称.

创建 archive 时,可以使用复制规范的全部功能,这意味着您可以执行内容过滤,文件重命名或上一节中介绍的其他任何操作. 一个特别常见的要求是将文件复制到源文件夹中不存在的 archive 的子目录中,这可以通过 `into()` <<#sub:using_child_copy_specifications,child specifications>> 来实现.
...
Gradle 关于 archive 的命名以及根据项目使用的插件在何处创建 archive 有一些约定. 基本约定由  <<base_plugin.adoc#base_plugin,Base Plugin>>, 提供,该默认插件默认在 `$buildDir/distributions`  目录中创建归档文件,并且通常使用格式为 _[projectName]-[version].[type]_ 的归档文件名称.
...
生成的 archive 的完整文件名.如果默认值中的任何属性为空,则会删除其 '-'  分隔符.
...
存档文件名的 classifier 部分.通常用于区分针对不同平台的 archive .
...
通常,您将需要发布 archive ,以便可以在另一个项目中使用它.<<artifact_management.adoc#artifact_management,旧版发布>>中介绍了此过程.