...
当应用于项目时，该插件会自动将 `gradleTestKit()` 依赖项添加到测试编译配置中。 此外，它会自动为测试中的代码生成类路径，并通过 link:{javadocPath}/org/gradle/testkit/runner/GradleRunner.html#withPluginClasspath--[GradleRunner.withPluginClasspath()] 为用户创建的任何 `GradleRunner`  实例注入该类路径。 需要特别注意的是，该机制目前仅在使用 <<plugins.adoc#sec:plugins_block,plugins DSL>> 应用被测插件时有效。 如果 <<#sub:gradle-runner-gradle-version,目标 Gradle 版本>>是 2.8 之前的版本，则不会执行自动插件类路径注入。
