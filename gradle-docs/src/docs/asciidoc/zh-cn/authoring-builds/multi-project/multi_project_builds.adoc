...
如果您使用 Gradle 构建一定规模的软件，您有两种基本的结构机制。首先，本章描述了如何使用 Gradle 多项目来构建您的软件项目。
在本文档中，我们认为这是一个内部结构化的单个 _软件组件_。 其次，您可以将您的软件视为由多个 _软件组件_ 组成的 _软件产品_，其中每个组件由一个单独的 Gradle 构建表示。 这在<<structuring_software_products.adoc#,structuring software products with Gradle>>一章中有详细描述
