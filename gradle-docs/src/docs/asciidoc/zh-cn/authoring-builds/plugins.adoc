...
* 包括来自定义为 buildscript 依赖项的外部 jar 中的插件（请参阅 <<#sec:applying_plugins_buildscript,Applying plugins using the buildscript block>>）。
...
插件DSL提供了一种声明插件依赖关系的简洁方便的方法。 它与 http://plugins.gradle.org[Gradle plugin portal] 一起使用，以提供对核心插件和社区插件的轻松访问。 插件 DSL 块配置 link:{javadocPath}/org/gradle/plugin/use/PluginDependenciesSpec.html[PluginDependenciesSpec] 的实例。
...
.仅在某些子项目上应用插件
====
include::{snippets-dir}/plugins/multiproject/groovy/project/settings.gradle[tags=include-subprojects];build.gradle[tags=plugins-on-subprojects];hello-a/build.gradle[];hello-b/build.gradle[];goodbye-c/build.gradle[]
include::{snippets-dir}/plugins/multiproject/kotlin/project/settings.gradle.kts[tags=include-subprojects];build.gradle.kts[tags=plugins-on-subprojects];hello-a/build.gradle.kts[];hello-b/build.gradle.kts[];goodbye-c/build.gradle.kts[]
====
...
====
include::{snippets-dir}/plugins/pluginManagement/groovy/settings.gradle[];init.gradle[]
include::{snippets-dir}/plugins/pluginManagement/kotlin/settings.gradle.kts[];init.gradle.kts[]
====
...
====
include::{snippets-dir}/plugins/pluginVersions/groovy/project/settings.gradle[tags=configure-plugin-version];build.gradle[tags=apply-plugin];gradle.properties"]
include::{snippets-dir}/plugins/pluginVersions/kotlin/project/settings.gradle.kts[tags=configure-plugin-version];build.gradle.kts[tags=apply-plugin];gradle.properties"]
====
...
插件解析规则可让您修改在 `plugins {}` 块中发出的插件请求，例如 更改请求的版本或明确指定实现工件坐标。
...
这告诉Gradle使用指定的插件实现构件，而不是使用其从插件 ID 到 Maven/Ivy 坐标的内置默认映射。

自定义 Maven 和 Ivy 插件存储库除了实际实现插件的工件外，还必须包含 <<#sec:plugin_markers,plugin marker artifacts>>。 有关将插件发布到自定义存储库的更多信息，请阅读 Gradle Plugin Development Plugin。
...
由于  `plugins {}`  DSL 块仅允许通过其全局唯一的插件 `id` 和 `version`  属性来声明插件，因此 Gradle 需要一种方法来查找插件实现工件的坐标。 为此，Gradle 将查找带有坐标  `plugin.id:plugin.id.gradle.plugin:plugin.version` 的插件标记工件。
