...
每个 Gradle 构建都由一个或多个 _projects_ 组成。 一个 project 代表什么取决于您使用 Gradle 做的事情。 例如，一个项目可能代表一个 JAR 库或一个 Web 应用程序。 它可能表示从其他项目产生的 JAR 组装而成的发行版 ZIP。
项目不一定代表要构建的事物。 它可能表示要完成的事情，例如将应用程序部署到暂存或生产环境。 暂时不要担心这似乎有点含糊。 Gradle 的按惯例构建支持为项目的定义添加了更具体的定义。
...
在命令行 shell 中，移至包含目录，并使用 `gradle -q hello` 执行构建脚本：

[TIP]
.`-q` 做什么？
====

本用户指南中的大多数示例都使用 `-q` 命令行选项运行。 这样可以取消Gradle的日志消息，从而仅显示任务的输出。 这样可以使本用户指南中的示例输出更加清晰。
...
这里发生了什么？ 此构建脚本定义了一个名为 `hello` 的任务，并向其中添加了一个操作。 当您运行 `gradle hello` 时，Gradle 将执行 `hello` 任务，该任务又将执行您提供的操作。 该动作只是一个包含一些要执行代码的块。

如果您认为这与 Ant 的 targets 相似，那是对的。 Gradle 任务等效于 Ant targets，但正如您将看到的那样，它们的功能要强大得多。 我们使用了与 Ant 不同的术语，因为我们认为 _task_ 一词比 _target_ 一词更具表现力。
不幸的是，这会引起与 Ant 的术语冲突，因为 Ant 会调用其命令（例如 `javac` 或 `copy`）任务。 因此，当我们谈论任务时，我们总是指Gradle任务，它等同于Ant的目标。 如果我们谈论Ant任务（Ant命令），则明确地说出 _Ant task_。
...
可以在定义 `taskY` 之前声明 `taskX` 对 `taskY` 的依赖关系。 对于多项目构建，这种自由非常重要。 将 <<more_about_tasks.adoc#sec:adding_dependencies_to_tasks,依赖添加到任务>>中将详细讨论任务依赖。
...
创建任务后，即可通过 API 对其进行访问。例如，您可以使用它在运行时为任务动态添加依赖项。蚂蚁不允许这样的事情。
...
您可以通过将依赖项添加到类路径配置中来声明构建脚本类路径。 这与您声明 Java 编译类路径的方式相同。 您可以使用除项目依赖项以外的任何 <<declaring_dependencies.adoc#sec:dependency-types,dependency types>>。
...
对于多项目构建，使用项目的 `buildscript()` 方法声明的依赖项可用于其所有子项目的构建脚本。

构建脚本依赖项可能是 Gradle 插件。请参阅 <<plugins.adoc#plugins,使用 Gradle 插件>>以获取有关 Gradle 插件的更多信息。

每个项目都会自动具有 link:{groovyDslPath}/org.gradle.api.tasks.diagnostics.BuildEnvironmentReportTask.html[BuildEnvironmentReportTask] 类型的 `buildEnvironment` 任务，可以调用该任务来报告构建脚本依赖项的解决方案。
