...
Groovy 构建脚本可以包含任何 Groovy 语言元素.footnote:[Any language element except for statement labels.] Kotlin 构建脚本可以包含任何 Kotlin 语言元素. Gradle 每个构建脚本都使用 UTF-8 编码.

[[sec:project_api]]
== Project API

在第七章 Java 构建入门那部分我们使用了 `apply()` 方法,这个方法是从哪里来的呢? 我们之前
说过 Gradle 在构建脚本中定义了一个项目. 对于构建脚本中每个项目,Gradle 都创建了一个 Project 类型的对象用来关联此项目. 当构建脚本执行时,它会去配置所关联的工程对象.
...
有关其他属性及其API的更多详细信息,请参阅 API 文档中的 link:{groovyDslPath}/org.gradle.api.plugins.ExtraPropertiesExtension.html[ExtraPropertiesExtension] 类.
