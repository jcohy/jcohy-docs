...
你可以使用 `.defaultIfEmpty(T)` 和 `.switchIfEmpty(Publisher<T>)` 将空序列替换为包含 `T` 类型缺省值的 `Publisher<T>` 序列(而不是 `Void` 序列)， 从而可以避免类似的情况出现。
...
以下示例展示了构建器的简单用法，在重试尝试延迟之前和之后，hook 都记录了消息，每次重试的间隔都会递增 (伪代码: delay = attempt number * 100 milliseconds):
...
无论如何，前面的辅助方法的用法可能类似于以下反应式 Web 控制器:
