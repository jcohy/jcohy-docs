...
NOTE: 默认情况下,应用程序是在单独的进程中启动的,并且使用 JMX 与应用程序进行通信. 如果需要配置 JMX 端口,请参见 <<integration-tests-example-jmx-port,the dedicated example>>.

您还可以配置更高级的设置,以在设置了特定属性时跳过集成测试,请参见 <<integration-tests-example-skip,the dedicated example>>.
