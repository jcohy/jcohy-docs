...
Classifier 添加到重新打包的 archive 中. 如果未给出,则 main artifact 将被重新打包的 archive 替换. 如果提供了 Classifier,则该 Classifier 还将用于确定要重新打包的源归档文件: 如果已经存在带有该分类器的 artifact,则它将用作源并被替换. 如果不存在这样的 artifact ,则 main artifact 将用作源,并且重新打包的存档将作为带有该分类器的补充 artifacts 进行附加. 附加 artifacts 可以将其与原始 artifacts 一起部署,请参见  $1[$2].
