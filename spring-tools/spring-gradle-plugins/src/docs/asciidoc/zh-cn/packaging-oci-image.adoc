...
如果构建器在其运行的 Docker 守护程序与构建打包下载工件的网络位置之间存在网络代理，则需要将构建器配置为使用代理。 使用 Paketo 构建器时，可以通过设置 `HTTPS_PROXY` 和/或 `HTTP_PROXY` 环境变量来实现，如以下示例所示：
...
==== 自定义镜像名称
默认情况下，映像名称是根据项目的  `name` 和  `version` 来推断的，例如 `docker.io/library/${project.name}:${project.version}`。 您可以通过设置任务属性来控制名称，如以下示例所示：
