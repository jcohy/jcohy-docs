...
如果要在归档中包括在 `developmentOnly` 配置中声明的依赖项，请配置其任务的类路径以包括该配置，如 `bootWar` 任务的以下示例所示：
...
`intoLayer` 块使用嵌套的 `include` 和 `exclude` 元素声明内容。 `application` 对 include/exclude  表达式使用 Ant 风格的匹配模式。 `dependencies` 使用 `group:artifact[:version]` 模式。

如果未定义 `include`，则将考虑所有内容（较早的块未声明）。

如果未定义 `exclude`，则不应用任何排除项。

查看上面的 `dependencies` 示例，我们可以看到第一个 `intoLayer` 将声明 `snapshot-dependencies` 层的所有 SNAPSHOT 依赖。 后续的 `intoLayer` 将声明 `dependencies` 层的所有剩余内容（在这种情况下，不是SNAPSHOT 的任何依赖）。
