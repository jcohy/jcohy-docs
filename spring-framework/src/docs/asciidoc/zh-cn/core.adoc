...
其中最重要的是 Spring 框架的控制反转(IoC) 容器.  之后,将全面介绍Spring的面向切面的编程(AOP) 技术.
Spring 框架拥有自己的 AOP 框架,该框架在概念上易于理解,并且成功解决了Java企业编程中AOP要求的80％的难题.

还提供了 Spring 与 AspectJ 的集成(目前,在功能上最丰富) 以及Java企业领域中最成熟的AOP实现) .
