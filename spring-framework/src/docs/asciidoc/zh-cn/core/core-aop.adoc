...
这假设您使用<<core.adoc#xsd-schemas-core, 基于 XML Schema 配置>>中描述的 schema 支持. 有关如何在 `aop` 命名空间中导入标签,请参阅 <<core.adoc#xsd-schemas-core-aop,  AOP schema>>.
...
要使用本节中描述的 `aop` 命名空间标签,您需要导入 `spring-aop` schema,如基于 XML 模式的配置中所述. 有关如何在 aop 命名空间中导入标记,请参阅<<core.adoc#xsd-schemas-core-aop, AOP schema>>.
...
如果基于 XML 配置,那么只要在 Spring<<core.adoc#xsd-schemas-core-context, `context` 命名空间>>声明中添加 `context:spring-configured`.
您可以按如下方式使用它:
