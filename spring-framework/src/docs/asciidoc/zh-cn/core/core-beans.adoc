...
总而言之,当调用 `emailService` bean的 `sendEmail()` 方法时,如果有任何应列入黑名单的电子邮件消息,则会发布 `BlockedListEvent` 类型的自定义事件.  `blockedListNotifier` bean 注册为 `ApplicationListener` 并接收 `BlockedListEvent` ,此时它可以通知相关方.
