...
可以使用 `new` 运算符调用构造函数. 除了基本类型和 String 外需要使用全限定类名 (`int`, `float`,等等是可以直接使用的).  以下示例显示如何使用 `new` 运算符来调用构造函数:
...
安全的引导运算符用于避免 `NullPointerException` 异常,这种观念来自 http://www.groovy-lang.org/operators.html#_safe_navigation_operator[Groovy] 语言. 当需要引用一个对象时, 可能需要在访问对象的方法或属性之前验证它是否为 `null`. 为避免出现这种情况, 安全引导运算符将简单地返回 `null`,而不是引发异常.
