...
为了使用更具声明性的方法和更简洁的语法,Spring 框架提供了 {docs-spring-framework}/kdoc-api/spring-framework/org.springframework.context.support/-bean-definition-dsl/[Kotlin bean 定义的 DSL],它通过简洁的声明性 API 声明了 `ApplicationContextInitializer`,能够处理配置文件和 `Environment` 自定义 bean 的注册方式. 在以下示例中注意:
...
* WebFlux {docs-spring-framework}/kdoc-api/spring-framework/org.springframework.web.reactive.function.client/index.html[client] 和  {docs-spring-framework}/kdoc-api/spring-framework/org.springframework.web.reactive.function.server/index.html[server] 功能 API 的扩展.
...
*  {docs-spring-framework}/kdoc-api/spring-framework/org.springframework.messaging.rsocket/index.html[`RSocketRequester`] 的扩展
...
这是通过  {docs-spring-framework}/kdoc-api/spring-framework/org.springframework.web.reactive.function.server/co-router.html[coRouter { }]  DSL 和相关处理程序定义的 Coroutines 路由器的示例.
