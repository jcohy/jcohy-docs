...
通过利用 `kotlin-script-runtime` 和 `scripting-jsr223-embedded` 的依赖,可以使用此类功能来使用 https://github.com/Kotlin/kotlinx.html[kotlinx.html] DSL 或 Kotlin 多行插值 `String` 渲染基于 Kotlin 的模板.
...
这是通过  {doc-root}/spring-framework/docs/{spring-framework-version}/kdoc-api/spring-framework/org.springframework.web.reactive.function.server/co-router.html[coRouter { }]  DSL 和相关处理程序定义的 Coroutines 路由器的示例.
