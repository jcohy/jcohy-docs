...
* <<rest-resttemplate>>: 具有 synchronous, template method API 的原始的 Spring REST 客户端.
* <<web-reactive.adoc#webflux-client, WebClient>>: 一种非阻塞的，响应式的替代方案，它支持同步,异步以及 streaming 场景.

NOTE: 从 5.0 版本开始，`RestTemplate` 处于维护模式，以后只有很小的更改和错误请求被接受。 请考虑使用更现代的支持同步,异步和流方案的 <<web-reactive.adoc#webflux-client, WebClient>> API.
...
默认情况下，`RestTemplate` 注册所有内置 <<rest-message-conversion, message converters>>，这取决于类路径检查，这些检查有助于确定存在哪些可选转换库。 您还可以将消息转换器设置为显式使用.
...
框架中提供了主要的媒体（MIME）类型的具体实现，默认情况下，这些实现在客户端的 `RestTemplate` 和服务器的 `RequestMethodHandlerAdapter` 进行了注册（请参阅 <<web.adoc#mvc-config-message-converters, Configuring Message Converters>>）。
...
要发送 multipart data，您需要提供一个 `MultiValueMap<String, Object>`，其值可能是一个对象的部分内容，一个资源的文件部分，或一个 `HttpEntity` 的部分内容的头。例如:
...
* <<remoting-web-services>>: Spring 通过 JAX-WS 提供对 Web 服务的远程支持.
* <<remoting-amqp>>: 单独的 Spring AMQP 项目支持使用 AMQP 作为底层协议进行远程处理.

WARNING: 从 Spring Framework 5.3 开始，出于安全原因和更广泛的行业支持，现已不支持多种远程技术。 支持的基础结构将从 Spring Framework 的下一个主要版本中删除.
...
[[remoting-amqp]]
=== AMQP
Spring AMQP 项目支持通过 AMQP 作为基础协议进行远程处理。 有关更多详细信息，请访问 Spring AMQP 参考的 {doc-spring-amqp}/html/#remoting[Spring Remoting].
...
`wsdlDocumentUrl` 是WSDL文件的URL. Spring 需要这样的启动时间来创建 JAX-WS服务. `namespaceUri` 对应于 `.wsdl` 文件中的 `targetNamespace` . `serviceName` 对应于.wsdl文件中的服务名称.  `portName` 对应于.wsdl文件中的端口名称.
...
WARNING: 从 Spring Framework 5.3 开始，RMI 支持已被弃用，不会被取代。
...
=== 使用 Hessian 通过 HTTP 远程调用服务 (已弃用)

WARNING: 从 Spring Framework 5.3 开始, Hessian 已经弃用，不会被取代.
...
WARNING: 从 Spring Framework 5.3 开始, HTTP 调用已经弃用，不会被取代.
...
WARNING: 从 Spring Framework 5.3 开始, JMS 已经弃用，不会被取代.
