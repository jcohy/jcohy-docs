...
具体来说,Spring的测试支持会在调用当前测试方法之前将事务状态绑定到当前线程 (通过 `java.lang.ThreadLocal` 变量) .  如果测试框架在新线程中调用当前测试方法以支持抢占式超时,则在当前测试方法内执行的任何操作都不会在测试管理的事务内调用.  因此,任何此类操作的结果都不会随着测试管理的事务而回滚.  相反,即使Spring适当地回滚了测试管理的事务,此类操作也将提交给持久存储 (例如关系数据库) .
