...
要发送多部分数据,您需要提供一个 `MultiValueMap<String, ?>` 其值可以是代表部件内容的 `Object` 实例或代表部件内容和 header 的 `HttpEntity` 实例. `MultipartBodyBuilder` 提供了方便的API来准备多部分请求. 下面的示例演示如何创建 `MultiValueMap<String, ?>`:
