...
* Servlet API Mocks: 用于单元测试控制器,过滤器和其他 Web 组件的 <<testing.adoc#mock-objects-servlet, Servlet API>> 契约的模拟实现.  有关更多详细信息,请参阅 Servlet API 模拟对象.
...
* `WebTestClient`: 专为测试 WebFlux 应用程序而构建,但它也可用于通过 HTTP 连接对任何服务器进行端到端集成测试.  它是一个非阻塞,响应式的客户端,非常适合测试异步和流式方案.
