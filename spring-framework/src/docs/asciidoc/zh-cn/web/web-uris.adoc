...
<<web-uricomponents, `UriComponentsBuilder`>> 实现了  `UriBuilder`.  您可以使用 `UriBuilderFactory` 创建一个 `UriBuilder`.  `UriBuilderFactory` 和 `UriBuilder` 一起提供了一种可插入机制,可以根据共享配置(例如基本 URL,编码首选项和其他详细信息) 从 URI 模板构建 URI.
