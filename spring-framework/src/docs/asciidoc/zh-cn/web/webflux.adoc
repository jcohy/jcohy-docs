...
最终效果与控制器返回 `RedirectView` 或 `Rendering.redirectTo("abc").build()` 相同,但是现在控制器本身可以根据逻辑视图名称进行操作. 视图名称(例如 `redirect:/some/resource` )相对于当前应用程序,而视图名称(例如 `redirect:https://example.com/arbitrary/path` )重定向到绝对 URL.
