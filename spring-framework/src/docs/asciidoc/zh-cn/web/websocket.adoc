...
<<websocket-stomp-appplication-context-events,Events>> 为 STOMP 连接的生命周期提供通知,但不为每个客户端消息提供通知.  应用程序还可以注册 `ChannelInterceptor` 来拦截任何消息以及处理链的任何部分.  以下示例显示如何拦截来自客户端的入站邮件:
