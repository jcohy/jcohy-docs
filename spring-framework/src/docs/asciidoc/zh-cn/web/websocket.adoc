...
NOTE: STOMP 代理中继总是在每个连接帧前设置 `login` 和 `passcode` 头,它转发给代理客户端. 因此,WebSocket 客户端无需设置这些 header ,它们也将被忽略.  正如<<websocket-stomp-authentication,身份验证>>部分所述,WebSocket 客户端应依赖 HTTP 身份验证来保护 WebSocket 端点并建立客户端标识.
