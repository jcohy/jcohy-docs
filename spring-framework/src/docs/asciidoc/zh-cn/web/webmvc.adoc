...
两者都有助于对模式进行排序，并将最匹配的放在顶层.当多个模式与 URL 匹配时,必须对它们进行比较以找到最佳匹配.  这是通过使用 `AntPathMatcher.getPatternComparator(String path)` 来完成的,它会查找更具体的模式.
...
从 5.3 开始，默认情况下 Spring MVC 不再执行 `.{asterisk}` 后缀模式匹配,以便映射到 `/person` 的控制器也隐式映射到 `/person.*`. 路径扩展不再用于解释请求的响应内容类型  - 例如,`/person.pdf`,`/person.xml` 等.
...
* 如果返回类型有multi-value流的语义,如 `Flux` (Reactor)或 `Observable` (RxJava),并且如果媒体类型也表示为流,(例如,`application/x-ndjson` 或 `text/event-stream`) .  则它是适配并等效于使用 `ResponseBodyEmitter` 或 `SseEmitter`. 还可以返回 `Flux<ServerSentEvent>` 或 `Observable<ServerSentEvent>`.
