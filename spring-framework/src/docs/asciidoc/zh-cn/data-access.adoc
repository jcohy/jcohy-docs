...
`@TransactionalEventListener` 注解暴露了一个phase属性,该属性允许您自定义监听器应绑定到的事务的阶段.  有效阶段是 `BEFORE_COMMIT`、`AFTER_COMMIT`(默认值) 、 `AFTER_ROLLBACK` 和 `AFTER_COMPLETION`,它们聚合事务完成(无论是提交还是回滚) .
