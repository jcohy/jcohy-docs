...
您可以在数据库中存储图片,其他二进制数据和大块文本.  这些大对象称为二进制数据的BLOB(二进制大对象) 和字符数据的CLOB(字符大对象) .  在Spring中,您可以直接使用 `JdbcTemplate` 处理这些大对象,也可以在使用RDBMS对象和 `JdbcTemplate` 类提供的更高抽象时处理这些大对象.
