...
或者您可以考虑通过 `BatchPreparedStatementSetter`（如前所示）； 或者通过给予基于 `List <Object[]>` 的调用的显式类型数组；或者通过在自定义 `MapSqlParameterSource` 的实例上 调用 `registerSqlType`； 或者通过从Java声明的属性类型扩展SQL类型- `BeanPropertySqlParameterSource`（即使对于空值）来显式指定相应的JDBC类型。
====
