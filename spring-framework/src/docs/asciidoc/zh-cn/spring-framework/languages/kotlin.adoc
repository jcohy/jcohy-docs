...
NOTE: 	请记住，Kotlin扩展需要被导入才能使用。这意味着，如果导入 `org.springframework.context.support.registerBean`，则 `GenericApplicationContext.registerBean` 扩展就可用。这样看来，与静态导入类似，IDE在大多数情况下应该建议自动导入。
