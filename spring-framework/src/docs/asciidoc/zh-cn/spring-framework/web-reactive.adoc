...
文档的介绍了基于 https://www.reactive-streams.org/[Reactive Streams] API构建的响应式 Web 应用程序的支持,该应用程序可在非阻塞服务器(例如 Netty,Undertow 和 Servlet 3.1+ 容器)上运行.
...
该注册表具有对 `RxJava和CompletableFuture` 的内置支持,但您也可以注册其他注册表.

对于功能性API(例如 <<webflux-fn>>,`WebClient` 等),WebFlux API 的一般规则适用于- `Flux` 和 `Mono` 作为返回值,而 Reactive Streams Publisher 作为输入.
提供发布者(无论是自定义的还是来自其他反应性库的发布者)时,只能将其视为语义未知(0..N)的流.但是,如果知道语义,则可以使用 `Flux` 或 `Mono.from(Publisher)` 对其进行包装,而不用传递原始 `Publisher`.
