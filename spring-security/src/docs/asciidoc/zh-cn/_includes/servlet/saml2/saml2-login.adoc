...
可配置的工厂.  只需在您的配置中将 `Saml2AuthenticationRequestFactory` 作为bean暴露即可.

端点将通过在可配置工厂上调用 `createAuthenticationRequest` 方法来生成 `AuthNRequest`.  只需在您的配置中将 `Saml2AuthenticationRequestFactory` 暴露为bean.
