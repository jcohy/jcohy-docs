...
如果为true且xss-protection-enabled为true,则将mode = block添加到 header .  这向浏览器指示根本不应加载该页面.  如果为false并且xss-protection-enabled为true,则在检测到反射攻击时仍将呈现页面,但是将修改响应以防止受到攻击.  请注意,有时存在绕过此模式的方法,这些方法常常使阻塞页面更为​​可取.
