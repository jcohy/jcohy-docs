...
此外，它还将 `OidcUser` 链接到 `OAuth2AuthorizedClient` 的简单实例，并将其存入 `HttpSessionOAuth2AuthorizedClientRepository`。 如果您的测试 <<testing-oauth2-client,使用 `@RegisteredOAuth2AuthorizedClient` 注解>>，则此方法很方便。
...
与 <<testing-testing-oidc-login,测试 OIDC 登录>> 一样，测试 OAuth 2.0 登录也面临着类似需要模拟授权流程挑战。 因此，Spring Security 还对非 OIDC 用例提供了测试支持。
...
此外，它还将 `OAuth2User` 链接到 `OAuth2AuthorizedClient` 的简单实例，该实例存储在 `HttpSessionOAuth2AuthorizedClientRepository` 中。 如果您的测试使用 <<testing-oauth2-client,使用 `@RegisteredOAuth2AuthorizedClient` 注解>> ，则这会很方便。
