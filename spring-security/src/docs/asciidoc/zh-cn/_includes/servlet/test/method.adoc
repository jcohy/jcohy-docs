...
NOTE: Spring Security使用 `WithSecurityContextTestExecutionListener` 吸引了Spring Test的支持,这将确保我们的测试以正确的用户运行.  它通过在运行我们的测试之前填充 `SecurityContextHolder` 来实现.  如果您正在使用响应式方法安全性,则还需要 `ReactorContextTestExecutionListener` 来填充 `ReactiveSecurityContextHolder`.
