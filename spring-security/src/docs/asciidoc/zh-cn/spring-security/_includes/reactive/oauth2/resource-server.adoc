...
[NOTE]
调用 `{security-api-url}org/springframework/security/oauth2/jwt/ReactiveJwtDecoders.html#fromIssuerLocation-java.lang.String-[ReactiveJwtDecoders#fromIssuerLocation]` 会调用提供者配置或授权服务器元数据终结点，以派生JWK设置Uri。 如果该应用程序未公开 `ReactiveJwtDecoder` Bean，则Spring Boot将公开上述默认值。
