...
* Simple Storage with <<servlet-authentication-inmemory,内存中的身份验证>>
* Relational Databases with <<servlet-authentication-jdbc,JDBC身份验证>>
* Custom data stores with <<servlet-authentication-userdetailsservice,使用 `UserDetailsService` 的自定义数据存储>>
* LDAP storage with <<servlet-authentication-ldap,LDAP认证的LDAP存储>>
