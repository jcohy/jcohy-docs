...
对于OAuth 2.0，仍然适用前面介绍的相同原则：最终，它取决于您所测试的方法期望位于“ SecurityContextHolder”中。
...
没有关于OAuth2的特定信息，因此您很可能可以简单地 <<test-method-withmockuser,使用 `@WithMockUser`>> 即可。

但是，在您的控制器绑定到Spring Security的OAuth 2.0支持的某些方面的情况下，如下所示：
...
然后Spring Security的测试支持就可以派上用场了。

[[testing-oidc-login]]
==== Testing OIDC Login

使用Spring MVC Test测试上述方法将需要使用授权服务器模拟某种授权流程。
当然这将是一项艰巨的任务，这就是为什么Spring Security附带支持删除此样板的原因。

例如，我们可以使用 `SecurityMockMvcRequestPostProcessors#oidcLogin` 方法告诉Spring Security包含默认的 `OidcUser`，如下所示：
...
这将为关联的 `MockHttpServletRequest` 配置一个 `OidcUser`，其中包括一个简单的 `OidcIdToken`，`OidcUserInfo` 和 `Collection` 的授权机构。

具体来说，它将包括一个 `OidcIdToken`，并将 `sub` 声明设置为 `user`：
...
以及只有一个权限 `SCOPE_read` 的权限的 `Collection`：
...
Spring Security做了必要的工作，以确保 `OidcUser` 实例可用于 <<mvc-authentication-principal,the `@AuthenticationPrincipal` annotation>>。

此外，它还将  `OidcUser`  链接到 `OAuth2AuthorizedClient` 的简单实例，并将其存入 `HttpSessionOAuth2AuthorizedClientRepository`。
如果您的测试<< testing-oauth2-client，使用 `@ RegisteredOAuth2AuthorizedClient` 注解>>，这将很方便。

[[testing-oidc-login-authorities]]
===== 配置权限

在许多情况下，您的方法受到过滤器或方法安全性的保护，并且需要您的 `Authentication`  具有某些授予的权限才能允许该请求。

在这种情况下，您可以使用  `authorities()`  方法提供所需的授权权限：
...
尽管授予的权限在整个Spring Security中非常普遍，但对于OAuth 2.0，我们也有主张。

例如，假设您有一个 `user_id` 声明，该声明指示系统中用户的ID。
您可以像在控制器中那样访问它：
...
因为 `OidcUser` 从 `OidcIdToken` 收集其 claims。

[[testing-oidc-login-user]]
===== 其他配置

还有其他方法可用于进一步配置身份验证。 它仅取决于您的控制器期望什么数据：

* `userInfo(OidcUserInfo.Builder)` - 用于配置 `OidcUserInfo` 实例
* `clientRegistration(ClientRegistration)` - 用给定的 `ClientRegistration` 配置关联的 `OAuth2AuthorizedClient`
* `oidcUser(OidcUser)` - 用于配置完整的 `OidcUser` 实例

如果您：
1.拥有自己的 `OidcUser` 实现，或
2.需要更改名称属性

例如，假设您的授权服务器在  `user_name`  声明中发送 principal 名称，而不是在  `sub` 声明中发送 principal 名称。
在这种情况下，您可以手动配置 `OidcUser`：
...
与 <<testing-oidc-login,testing OIDC login>> 一样，测试OAuth 2.0 Login也面临着类似模拟授权流程的挑战。
因此，Spring Security还对非OIDC用例提供了测试支持。
...
在这种情况下，我们可以使用  `SecurityMockMvcRequestPostProcessors#oauth2User` 方法告诉Spring Security包含默认的 `OAuth2User`，如下所示：
...
要做的是用一个OAuth2User来配置相关的 `MockHttpServletRequest`，该 `OAuth2User` 包括一个简单的属性Map和被授予权限的 Collection。

具体来说，它将包括一个带有  `sub`/`user` 键/值对的 `Map` ：
...
Spring Security做了必要的工作，以确保 `OAuth2User` 实例可用于 <<mvc-authentication-principal, `@AuthenticationPrincipal` 注解>>。

此外，它还将 `OAuth2User` 链接到一个简单的 `OAuth2AuthorizedClient` 实例，该实例存储在 `HttpSessionOAuth2AuthorizedClientRepository` 中。
如果您的测试 <<testing-oauth2-client,use the `@RegisteredOAuth2AuthorizedClient` 注解>>，这将很方便。
...
例如，假设您有一个 `user_id` 属性，该属性指示系统中用户的ID。
您可以像在控制器中那样访问它：
...
如果您：
1.拥有自己的 `OAuth2User` 实现，或者
2.需要更改名称属性

例如，假设您的授权服务器在  `user_name`  声明中发送 principal 名称，而不是在  `sub` 声明中发送 principal 名称。
在这种情况下，您可以手动配置  `OAuth2User` ：
...
与用户的身份验证方式无关，对于正在测试的请求，您可能还有其他令牌和客户端注册在起作用。
例如，您的控制器可能依赖于客户端凭据授予来获得与用户完全不相关的令牌：
...
使用授权服务器模拟此握手可能很麻烦。
相反，您可以使用  `SecurityMockMvcRequestPostProcessor#oauth2Client` 将 `OAuth2AuthorizedClient` 添加到 `HttpSessionOAuth2AuthorizedClientRepository` 中：
...
这将创建一个具有简单的 `ClientRegistration`，`OAuth2AccessToken` 和资源所有者名称的 `OAuth2AuthorizedClient`。

具体来说，它将包含一个  `ClientRegistration`，其客户端ID为  "test-client"，客户端密码为  "test-secret"：
...
还有一个 `OAuth2AccessToken`，它只有一个范围，即 `read`:
...
Spring Security做了必要的工作，以确保 `OAuth2AuthorizedClient` 实例在关联的 `HttpSession` 中可用。
这意味着可以从 `HttpSessionOAuth2AuthorizedClientRepository` 中检索它。

[[testing-oauth2-client-scopes]]
===== 配置 Scopes

在许多情况下，OAuth 2.0访问令牌都带有一组范围。
如果您的控制器检查了这些，请这样说：
...
例如，假设您要使用在 `application.yml` 中指定的应用程序的 `ClientRegistration` 定义之一。
...
类似于 <<testing-jwt,JWTs>>，不透明令牌需要授权服务器才能验证其有效性，这会使测试更加困难。
为了解决这个问题，Spring Security提供了对不透明令牌的测试支持。

假设我们有一个控制器，将控制器作为 `BearerTokenAuthentication` 检索身份验证：
...
在这种情况下，我们可以使用  `SecurityMockMvcRequestPostProcessors#opaqueToken`  方法告诉Spring Security包含默认的 `BearerTokenAuthentication`，如下所示：
...
要做的是用一个 `BearerTokenAuthentication` 配置关联的 `MockHttpServletRequest`，其中包括一个简单的 `OAuth2AuthenticatedPrincipal`，属性 `Map` 和授予权限的 `Collection`。
...
Spring Security做了必要的工作以确保 `BearerTokenAuthentication` 实例可用于您的控制器方法。

[[testing-opaque-token-authorities]]
===== 配置权限

在许多情况下，您的方法受到过滤器或方法安全性的保护，并且需要您的“身份验证”具有某些授予的权限才能允许该请求。
...
尽管授予的权限在整个Spring Security中非常普遍，但在OAuth 2.0的情况下，我们也具有属性。
...
其中一种就是  `principal(OAuth2AuthenticatedPrincipal)`，您可以使用它来配置作为 `BearerTokenAuthentication` 基础的完整的 `OAuth2AuthenticatedPrincipal` 实例。

如果您这样：
1.拥有自己的 `OAuth2AuthenticatedPrincipal` 实现，或者
2.要指定其他 principal 名称

例如，假设您的授权服务器在  `user_name` 属性而不是 `sub` 属性中发送 principal 名称。
在这种情况下，您可以手动配置一个 `OAuth2AuthenticatedPrincipal`：
...
注意，除了使用  `opaqueToken()`  测试支持之外，您还可以使用 `@MockBean` 注释来模拟  `OpaqueTokenIntrospector`  bean本身。

=== SecurityMockMvcRequestBuilders

Spring MVC Test还提供了一个 `RequestBuilder` 接口，该接口可用于创建测试中使用的 `MockHttpServletRequest`。 Spring Security提供了一些 `RequestBuilder` 实现，可用来简化测试。
