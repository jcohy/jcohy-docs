...
Spring Security的可传递依赖解决了Spring Framework 5.2.1.RELEASE的事实，这可能会引起奇怪的类路径问题，因此许多用户可能会误以为是。 解决此问题的最简单方法是在 `pom.xml` 的 `<dependencyManagement>` 部分中使用 `spring-framework-bom`，如以下示例所示：
...
前面的示例确保Spring Security的所有传递依赖都使用Spring 5.2.1.RELEASE模块。
