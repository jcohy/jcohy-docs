...
<1> 在运行应用程序的剩余部分之前，`SecurityContextPersistenceFilter` 从 `SecurityContextRepository` 加载 `SecurityContext` 并将其设置在 `SecurityContextHolder` 上。
<2> 接下来，运行应用程序。
...
<1> 在运行应用程序的剩余部分之前，`SecurityContextHolderFilter` 从 `SecurityContextRepository` 加载 `SecurityContext` 并将其设置在 `SecurityContextHolder` 上。
<2> 接下来，运行应用程序。
