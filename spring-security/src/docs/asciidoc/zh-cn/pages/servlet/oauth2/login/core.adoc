...
* <<servlet-oauth2login-sample-initial-setup,初始设置>>
* <<servlet-oauth2login-sample-redirect-uri,设置重定向 URI>>
* <<servlet-oauth2login-sample-application-config,配置 application.yml>>
* <<servlet-oauth2login-sample-boot-application,启动应用程序>>
...
IMPORTANT: 如果 OAuth 客户端在代理服务器 (例如: Nginx) 后面运行,建议检查<<http-proxy-server, 代理服务器配置>>以确保正确配置了应用程序.  另外,请参阅 `redirect-uri` 支持的<<servlet-oauth2Client-auth-code-redirect-uri, `URI` 模板变量>>.
...
* <<servlet-oauth2login-register-clientregistrationrepository-bean,注册一个 `ClientRegistrationRepository` `@Bean`>>
* <<servlet-oauth2login-provide-websecurityconfigureradapter,提供一个 `WebSecurityConfigurerAdapter`>>
* <<servlet-oauth2login-completely-override-autoconfiguration,完全覆盖自动配置>>
...
[[servlet-oauth2login-provide-websecurityconfigureradapter]]
