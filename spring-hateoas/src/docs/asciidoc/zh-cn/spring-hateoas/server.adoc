...
通过在 Spring MVC 配置中激活 `@EnableHypermediaSupport`,可以通过依赖项注入来使用 `EntityLinks`. 这将导致注册 `EntityLink` 的各种默认实现. 最基础的是检​​查 SpringMVC 控制器类的 `ControllerEntityLinks`. 如果要注册自己的 `EntityLinks` 实现,请查看 <<server.entity-links.spi, 本节>>.
