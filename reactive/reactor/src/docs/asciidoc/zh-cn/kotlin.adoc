...
尽管 Java 的类型系统不允许这样的 `null` 值安全的表达方式, <<advancedFeatures.adoc#null-safety,now provides null safety>> 对所有 Reactor API 通过工具友好的(tooling-friendly)注解(在 `reactor.util.annotation` 包中定义)来支持.
