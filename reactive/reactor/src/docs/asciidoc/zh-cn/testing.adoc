...
----
<1> 如果你使用了 <<gettingStarted.adoc#getting,BOM>> ,你不需要指定 `<version>`.
====
...
`verifyThenAssertThat()` 返回一个 `StepVerifier.Assertions` 对象,你可以用它来校验 整个测试场景成功刚结束后的一些状态(它也会调用 `verify()`).典型应用就是校验有多少 元素被操作符丢弃(参考 <<advancedFeatures.adoc#hooks,Hooks>>).

== 测试 `Context`

更多关于 `Context` 的内容请参考 <<advancedFeatures.adoc#context>>.
