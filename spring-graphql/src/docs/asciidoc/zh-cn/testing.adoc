...
创建 `HttpGraphQlTester` 后，您可以开始使用相同的 API <<testing-requests, execute requests>> ，而不受底层传输的影响。 如果您需要更改任何特定于传输的详细信息，请在现有 `HttpSocketGraphQlTester` 上使用 `mutate()` 来创建具有自定义设置的新实例：
