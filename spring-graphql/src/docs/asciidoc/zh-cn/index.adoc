...
如果存储库使用 `@GraphQlRepository` 注解，它会自动注册为尚未注册 `DataFetcher` 且其返回类型与存储库域类型匹配的查询。 这包括单值和多值查询。

默认情况下，查询返回的 GraphQL 类型的名称必须与存储库域类型的简单名称相匹配。 如果需要，您可以使用 `@GraphQlRepository` 的 `typeName` 属性来指定目标 GraphQL 类型名称。

自动注册检测给定的存储库是否实现了 `QuerydslBinderCustomizer` 并通过 `QuerydslDataFetcher` builder 方法透明地应用它。
...
Spring Data 支持使用 https://docs.spring.io/spring-data/commons/docs/current/reference/html/#query-by-example[Query by Example] 来获取数据。 示例查询 (QBE) 是一种简单的查询技术，不需要您通过商店特定的查询语言编写查询。
...
`QueryByExampleDataFetcher` 支持接口和 DTO 投影以在返回查询结果以供进一步 GraphQL 处理之前转换查询结果。
...
出现的一个常见问题是，GraphQL 选择集与 https://docs.spring.io/spring-data/commons/docs/current/reference/html/#projections[Spring Data projections] 相比如何，每个角色扮演什么角色？

简短的回答是，Spring for GraphQL 不是将 GraphQL 查询直接转换为 SQL 或 JSON 查询的数据网关。 相反，它允许您利用现有的 Spring 技术，并且不假设 GraphQL 模式和底层数据模型之间存在一对一的映射。 这就是数据模型的客户端驱动选择和服务器端转换可以起到互补作用的原因。
...
在其他情况下，减少甚至转换底层数据模型以适应 GraphQL 模式是很有用的。 Spring Data 通过接口和 DTO 投影支持这一点。
...
Spring for GraphQL 提供了一种基于注解的编程模型，其中 `@Controller` 组件使用注解来声明具有灵活方法签名的处理程序方法，以获取特定 GraphQL 字段的数据。 例如：
...
您可以将 `@Controller` bean 定义为标准的 Spring bean 定义。 `@Controller` 构造型允许自动检测，与 Spring 对检测类路径上的 `@Controller` 和 `@Component` 类并为它们自动注册 bean 定义的一般支持保持一致。
它还充当带注解类的构造型，表明其在 GraphQL 应用程序中作为数据获取组件的角色。

`AnnotatedControllerConfigurer` 检测 `@Controller` bean 并通过 `RuntimeWiring`.Builder 将其注解的处理程序方法注册为 `DataFetchers`。 它是 `RuntimeWiringConfigurer` 的一个实现，
可以添加到 `GraphQlSource.Builder`。 Spring Boot starter 自动将 `AnnotatedControllerConfigurer` 声明为一个 bean，并将所有 `RuntimeWiringConfigurer` beans 添加到 `GraphQlSource.Builder` 并启用对带注解的 `DataFetchers` 的支持，请参阅 Boot starter 文档中的 {spring-boot-ref-docs}/web.html#web.graphql.runtimewiring[GraphQL RuntimeWiring] 部分。

[[controllers-schema-mapping]]
=== `@SchemaMapping`

`@SchemaMapping` 注解将处理程序方法映射到 GraphQL 模式中的字段，并将其声明为该字段的 `DataFetcher`。 注解可以指定父类型名称和字段名称：
...
`@SchemaMapping` 注解也可以省略那些属性，在这种情况下，字段名默认为方法名，而类型名默认为注入到方法中的源/父对象的简单类名。 例如，下面默认键入 "Book" 和字段 "author"：
...
使用 `@Argument` 注解将参数绑定到目标对象并注入到处理程序方法中。 绑定是通过将参数值映射到预期方法参数类型的主数据构造函数来执行的，或者通过使用默认构造函数来创建对象，然后将参数值映射到它的属性来执行。
...
TIP: `@Argument` 注解没有 "required" 标志，也没有指定默认值的选项。 这两者都可以在 GraphQL 模式级别指定，并由 GraphQL Java 强制执行。
...
作为使用带有 <<controllers-schema-mapping-argument>> 的完整对象的替代方法，您还可以使用投影接口通过定义良好的最小接口访问 GraphQL 请求参数。 当 Spring Data 在类路径上时，参数投影由 https://docs.spring.io/spring-data/commons/docs/current/reference/html/#projections.interfaces[Spring Data's Interface projections] 提供。

要使用它，请创建一个用 `@ProjectedPayload` 注解的接口并将其声明为控制器方法参数。 如果参数用 `@Argument` 注解，它适用于 `DataFetchingEnvironment.getArguments()` 映射中的单个参数。
...
在 GraphQL Java 中，`DataFetchingEnvironment` 提供对字段的源 (i.e. parent/container) 实例的访问。 要访问它，只需声明一个预期目标类型的方法参数。
...
作为回退，`DataLoader` 方法参数解析器还将尝试将方法参数名称作为键，但通常这不是必需的。
...
当找到 `javax.validation.Validator` bean 时，`AnnotatedControllerConfigurer` 启用对带注解的控制器方法的 {spring-framework-ref-docs}/core.html#validation-beanvalidation-overview[Bean Validation]。 通常，bean 的类型为 `LocalValidatorFactoryBean`。
...
以上成为 `BatchLoaderRegistry` 中的批量加载函数，其中键是 `Book` 实例，加载的值是它们的作者。 此外，`DataFetcher` 还透明地绑定到 `Book` 类型的作者字段，它简单地委托给作者的 `DataLoader`，
