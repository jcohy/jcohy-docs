...
include::code:CityRepository.java[]
...
include::code:CountryRepository.java[]
...
include::code:DevProfileSecurityConfiguration.java[tag=!customizer]
...
include::code:MyBean.java[tag=!method]
...
include::code:MyR2dbcConfiguration.java[]

以下示例显示了如何设置一些 PostgreSQL 连接选项:

include::code:MyPostgresR2dbcConfiguration.java[]
