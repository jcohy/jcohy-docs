...
默认情况下，ActiveMQ 自动配置为使用 https://activemq.apache.org/vm-transport-reference.html[VM transport]，它启动嵌入在同一 JVM 实例中的代理。
...
如果您想完全控制嵌入式代理，请参阅 https://activemq.apache.org/how-do-i-embed-a-broker-inside-a-connection.html[ActiveMQ 文档] 了解更多信息 .
