...
如果你在你的应用程序中使用了 `@ConfigurationProperties` 注解，请确保配置 `spring-boot-maven-plugin`，以防止 `repackage` 目标将依赖添加到 jar 中:
...
如果该类也使用了  `@ConstructorBinding` 注解，则应使用单个构造函数，并为每个构造函数参数创建一个属性。否则,将通过存在标准的 `getter` 和 `setter` 并对集合和 Map 类型进行特殊处理来发现属性(即使仅存在getter 也会被检测到)。注解处理器还支持使用 `@Data`,`@Getter` 和 `@Setter` lombok注解.
...
这暴露了三个属性，其中 `server.name` 没有默认值，`server.ip` 和 `server.port` 分别默认为 `"127.0.0.1"` 和 `9797`。
字段值的 Javadoc 用于填充 `description` 属性.。 例如，`server.ip` 的描述是 "IP address to listen to"。
...
对于无法检测到默认值的情况,应提供<<configuration-metadata-additional-metadata,手动元数据>>.
