...
==== 自定义 Serializer（序列化） 和 Deserializer（反序列化）
...
您可以直接在 `JsonSerializer`, `JsonDeserializer` 或 `KeyDeserializer`  实现类上使用 `@JsonComponent` 注解. 您也可以在将序列化器/反序列化器 (deserializer) 作为内部类的类上使用. 例如:
...
上面的示例可以使用 `JsonObjectSerializer`/`JsonObjectDeserializer` 重写，如下所示:

include::code:object/MyJsonComponent[]

[[features.json.jackson.mixins]]
==== Mixins
Jackson 支持 mixins，可以将其他注解混合到已在目标类上声明的注解中。 Spring Boot 的 Jackson 自动配置将扫描应用程序的包以查找带有 `@JsonMixin` 注解的类，并将它们注册到自动配置的 `ObjectMapper`。 注册由 Spring Boot 的 `JsonMixinModule` 执行。
