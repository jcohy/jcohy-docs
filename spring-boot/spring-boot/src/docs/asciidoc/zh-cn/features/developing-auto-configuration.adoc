...
TIP: 这里有一个 https://github.com/snicoll-demos/spring-boot-master-auto-configuratio[演示项目] 展示了如何逐步创建 starter.

[[features.developing-auto-configuration.understanding-auto-configured-beans]]
=== 理解自动配置的 Beans
在内部,自动配置是通过 `@AutoConfiguration` 注解实现的。这个注解本身是用 `@Configuration` 元注解的，使自动配置成为标准的 `@Configuration` 类。
...
您可以浏览  {spring-boot-autoconfigure-module-code}[`spring-boot-autoconfigure`] 的源代码,以查看 Spring 提供的 `@Configuration`  类 (请参阅   {spring-boot-code}/spring-boot-project/spring-boot-autoconfigure/src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports[`META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`] 文件) .

[[features.developing-auto-configuration.locating-auto-configuration-candidates]]
=== 找到候选的自动配置

Spring Boot 会检查已发布 jar 中是否存在 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports` 文件. 该文件应列出 `EnableAutoConfiguration` key 下的配置类,如下所示:
...
TIP: 如果您使用 {spring-boot-autoconfigure-module-code}/AutoConfiguration.java[`@AutoConfiguration`] 注解，可以使用 `before`、`beforeName`、`after` 和 `afterName` 属性别名而不是专用的注解。
