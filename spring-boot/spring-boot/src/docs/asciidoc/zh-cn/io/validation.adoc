...
解析 constraint messages 中的 `+{parameters}+` 时使用应用程序的 `MessageSource`。
这允许您使用 <<features.adoc#features.internationalization,您的应用程序的 `messages.properties` 文件>> 用于 Bean 验证消息。
