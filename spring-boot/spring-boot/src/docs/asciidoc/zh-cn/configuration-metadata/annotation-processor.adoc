...
[NOTE]
====
如果你项目中使用 Lombok，你需要保证此注解处理器在 `spring-boot-configuration-processor` 注解处理器之前运行。
如果您使用的是 Maven，您可以使用 Maven 编译器插件的 `annotationProcessors` 属性按顺序列出注解处理器。
如果你没有使用这个属性，并且可以从类路径上获取注解处理器，请确保在 `spring-boot-configuration-processor` 依赖之前定义 `lombok` 依赖。
====
...
如果该类也使用了  `@ConstructorBinding` 注解,则应使用单个构造函数,并为每个构造函数参数创建一个属性.否则,将通过存在标准的 `getter` 和 `setter` 并对集合和 Map 类型进行特殊处理来发现属性(即使仅存在 getter 也会被检测到).注解处理器还支持使用 `@Data`, `@Value`, `@Value``@Getter` 和 `@Setter` lombok 注解.
