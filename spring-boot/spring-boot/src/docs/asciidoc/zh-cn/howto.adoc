...
如果已为应用程序启用了Spring MVC,则Spring Data REST可以为您将 `Repository` 实现作为REST端点暴露.

Spring Boot暴露了一组有用的属性 (来自 `spring.data.rest` 命名空间) ,这些属性来自定义 {spring-data-rest-api}/core/config/RepositoryRestConfiguration.html[`RepositoryRestConfiguration`].  如果需要提供其他定制,则应使用 {spring-data-rest-api}/webmvc/config/RepositoryRestConfigurer.html[`RepositoryRestConfigurer`] bean.
