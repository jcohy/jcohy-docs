...
* 对于reactive 技术栈的应用,`spring-boot-starter-webflux` 包括响应式堆栈的 Netty 通过包括 `spring-boot-starter-reactor-netty`,但你可以使用 `spring-boot-starter-tomcat`,`spring-boot-starter-jetty` 或 `spring-boot-starter-undertow` 代替.
