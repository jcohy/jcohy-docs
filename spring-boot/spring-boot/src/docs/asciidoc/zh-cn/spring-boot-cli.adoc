...
NOTE: 默认元数据与您使用的 CLI 版本相关.  仅当您移至新版本的 CLI 时,它才会更改,从而使您可以控制依赖的版本何时更改.  可以在 <<appendix-dependency-versions.adoc#dependency-versions,附录中>>找到一个表格,其中显示了默认元数据中包含的依赖及其版本.
