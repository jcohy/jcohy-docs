...
最后，提供了一个  "starter"  依赖项，以帮助用户尽可能轻松地入门。

具体而言，自定义启动器可以包含以下内容：

* `autoconfigure` 模块,为 "acme" 包含自动配置代码.
* starter 模块,它为 "acme" 提供对 `autoconfigure` 模块依赖关系以及类库和常用的其他依赖关系. 简而言之,添加 starter 应该提供该库开始使用所需的一切依赖.

完全没有必要将这两个模块分开.
如果 "acme" 具有多种功能，选项或可选功能，则最好将自动配置分开，这样您可以清楚地表示某些功能是可选的.此外，您还可以制作一个启动器，以提供有关可选的依赖项.
