...
解析这些文件并填充环境时,不会处理来自 <<boot-features-external-config-application-property-files, 应用程序属性文件>> 的 SpEL 表达式. 但是,可以在 `@Value` 中编写 SpEL 表达式. 如果 <<boot-features-external-config-application-property-files,应用程序属性文件>> 中的属性值是 SpEL 表达式,则在通过 `@Value` 进行使用时将对其进行评估.
