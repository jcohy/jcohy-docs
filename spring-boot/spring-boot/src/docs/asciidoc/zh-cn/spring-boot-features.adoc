...
如果嵌入式驱动程序和 Neo4j 内核位于上述类路径上,则 <<boot-features-testing-spring-boot-applications-testing-autoconfigured-neo4j-test,Data Neo4j Tests>>会自动使用嵌入式 Neo4j 实例.

NOTE: 您可以通过在配置中提供数据库文件路径来为内嵌模式启用持久化,例如: `spring.data.neo4j.uri=file://var/tmp/graph.db`.

[[boot-features-neo4j-ogm-native-types]]
==== 使用本地类型
Neo4j-OGM 可以将某些类型 (例如  `java.time.*` 中的类型) 映射到基于 `String` 的属性或 Neo4j 提供的本机类型之一.  出于向后兼容的原因,Neo4j-OGM 的默认设置是使用基于字符串的表示形式.  要使用本地类型,请添加对 `org.neo4j:neo4j-ogm-embedded-native-types` 或 `org.neo4j: neo4j-ogm-embedded-native-types` 的依赖,并配置 `spring.data.neo4j.use-native-types` 属性,如以下示例所示:
