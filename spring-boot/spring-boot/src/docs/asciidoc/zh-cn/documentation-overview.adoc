...
== 从旧版本升级
{github-wiki}[wiki] 项目提供了有关如何从早期版本的 Spring Boot 升级的说明.

按照 {github-wiki}#release-notes[release notes]  中的链接查找你要升级到那个版本.

升级说明始终是发行说明中的第一项.如果您落后多个发行版，请确保您还查看了所跳版本的发行说明.

您应该始终确保运行的是 Spring Boot {github-wiki}/Supported-Versions[支持的版本].
