...
还可以通过为依赖项以及应用程序类和资源（通常会更频繁地更改）<<spring-boot-features.adoc#boot-features-container-images-docker,creating separate layers>> 来创建更有效的容器映像。
...
要在 App Engine 中运行,您可以先在用户界面中创建一个项目,该项目将为您设置一个唯一的标识符,并还设置HTTP路由.  将Java应用程序添加到项目中,并将其保留为空,然后使用  https://cloud.google.com/sdk/install[Google Cloud SDK] 从命令行或CI构建将Spring Boot应用程序推送到该插槽中.
