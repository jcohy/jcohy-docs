...
当在类路径上存在 Prometheus Pushgateway 依赖并且 configprop:management.metrics.export.prometheus.pushgateway.enabled[] 属性设置为 `true` 时,Spring Boot 会自动配置 `PrometheusPushGatewayManager` bean.  这可以管理将指标推送到 Prometheus Pushgateway.
...
如果指标标准依赖于其他 bean，建议您使用 `MeterBinder` 进行注册，如以下示例所示:
...
使用 `MeterBinder` 可以确保建立正确的依赖关系，并且当获取指标值时 Bean 可用.

如果你发现跨组件或应用程序重复记录一套指标,则可以将此套件封装在 `MeterBinder` 实现中. 默认情况下,所有 `MeterBinder` bean 的指标都将自动绑定到 Spring 管理的 `MeterRegistry`.
