...
| `heapdump`
| 返回一个 `hprof` 堆 dump 文件.  需要一个 HotSpot JVM.
...
使用 `MeterBinder` 可以确保设置正确的依赖关系,并且在获取指标值时 Bean 可用. 如果你发现跨组件或应用程序重复记录一套指标,则 `MeterBinder` 实现也可能很有用.

NOTE:: 默认情况下,所有 `MeterBinder` bean 的指标都将自动绑定到 Spring 管理的 `MeterRegistry`
...
NOTE: 默认情况下, 所有的 `MeterFilter` 将自动绑定到由 Spring 管理的 `MeterRegistry`.
确保使用 Spring 管理的 `MeterRegistry` 而不是 `Metrics` 上的任何静态方法来注册您的指标。 这些使用非 Spring 管理的全局注册表。
