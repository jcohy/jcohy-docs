...
对于 SAML2 注销，默认情况下，Spring Security 的 `Saml2LogoutRequestFilter` 和 `Saml2LogoutResponseFilter` 只处理匹配 `/logout/saml2/slo` 的 URL。
如果您想自定义 AP 发起的注销请求发送到的 `url` 或 AP 发送注销响应的 `response-url`，要使用不同的模式，您需要提供配置来处理该自定义 图案。
例如，对于 servlet 应用程序，您可以添加自己的 `SecurityFilterChain`，如下所示：
