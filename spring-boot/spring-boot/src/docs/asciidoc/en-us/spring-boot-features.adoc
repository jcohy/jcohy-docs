...
`SpEL` expressions from   are not processed at time of parsing these files and populating the environment.
...
Generally, you provide the , username, and password, as shown in the following example:
...
==== Couchbase
If Spring Data Couchbase is available and Couchbase is , a `CouchbaseCacheManager` is auto-configured.
...
Alternatively, additional auto-configurations can be added for any use of a slice annotation by registering them in  as shown in the following example:
