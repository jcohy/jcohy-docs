...
== 系统要求
Spring Boot {spring-boot-version} 需要 https://www.java.com[Java 8] 并且与Java 13（包括）兼容。 还需要 {spring-framework-docs}[Spring Framework {spring-framework-version}]  或更高版本。
...
== 安装 Spring Boot
Spring Boot 可以与经典的 Java 开发工具一起使用或者作为命令行工具安装。无论如何，您将需要 https://www.java.com[Java SDK v1.8] 或者更高版本。在开始之前您应该检查 Java 的安装情况：
...
==== 使用 Gradle 安装
Spring Boot 兼容 Gradle 2 (2.9 或者更高版本)和 Gradle 3。如果您还没有安装 Gradle，您可以按照 https://gradle.org 上的说明进行操作。
...
=== 安装 Spring Boot CLI
Spring Boot CLI 是一个命令行工具，如果您想使用 Spring 快速搭建原型，可以选择它。它允许您运行 http://groovy-lang.org/[Groovy] 脚本，这意味着您有可以有类 Java 语法且没有太多样板的代码。
...
下载之后，请按照解压缩归档文件中的 {github-raw}/spring-boot-project/spring-boot-cli/src/main/content/INSTALL.txt[INSTALL.txt] 说明进行操作。总之：在 `.zip` 文件的 bin/ 目录中有一个 `spring` 脚本（在 Windows 下为 `spring.bat`），或者也可以使用 `java -jar` 配合 `.jar` 文件（该脚本可以帮助您确保 classpath 设置正确）。
...
== 下一步
希望您在本章节学到了一些 Spring Boot 的基础知识，并且开始编写自己的应用。如果您是一名面向任务（task-oriented）的开发人员，您可能想跳过 https://spring.io 而直接阅读一些能解决如何使用 Spring 来解决？ 这类问题的https://spring.io/guides/[入门指南]，此外我们还有 Spring Boot 专门的 "`<<howto.adoc#howto, How-to>>`" 参考文档。

Spring Boot 仓库还有很多您可以运行的示例。示例与其余部分的代码是独立的（也就是说，您不需要构建其他的代码来运行或使用示例）。
