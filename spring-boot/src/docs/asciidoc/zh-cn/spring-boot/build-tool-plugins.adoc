...
Spring Boot为Maven和Gradle提供了构建工具插件。 插件提供了多种功能，包括可执行jar的打包。 本节提供了有关这两个插件的更多详细信息，以及在扩展不受支持的构建系统时所需的一些帮助。
...
Spring Boot Maven 插件在Maven中提供了Spring Boot支持，使您可以打包可执行jar或war归档文件并 "`in-place`" 运行应用程序。 要使用它，必须使用Maven 3.2（或更高版本）。

NOTE: 请参阅 spring-boot-maven-plugin-docs}[Spring Boot Maven插件网站]以获取完整的插件文档。

[[build-tool-plugins-include-maven-plugin]]
=== 导入插件
要使用Spring Boot Maven插件，请在 `pom.xml` 的 `plugins` 部分中包含适当的XML，如以下示例所示：
...
Spring Boot Gradle插件在Gradle中提供了Spring Boot支持，可让您打包可执行jar或war归档文件，运行Spring Boot应用程序以及使用所提供的依赖管理 `spring-boot-dependencies`。
