...
WARNING:由于它在 Gradle 6 中被弃用，这个插件对使用 Gradle 的 `maven` 插件发布的支持已被弃用，并将在未来的版本中删除。 请改用 `maven-publish` 插件。

应用了 {maven-plugin}[`maven` plugin] ,将自动创建名为 `uploadBootArchives` 的 `bootArchives` 配置的 `Upload` 任务. 默认的, `bootArchives` 配置包含了 `bootJar` 或者 `bootWar` 任务产生文件.
`uploadBootArchives` 任务可以这么配置来发布到 Maven repository 里:
