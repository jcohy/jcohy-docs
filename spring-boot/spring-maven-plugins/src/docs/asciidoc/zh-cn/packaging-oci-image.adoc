...
可以设置环境变量来配置 `build-image` 目标以使用 https://minikube.sigs.k8s.io/docs/tasks/docker_daemon/[minikube 提供的 Docker 守护进程]。 . 下表显示了环境变量及其值:
...
在 Linux 和 macOS 上，可以在 minikube 启动后使用命令 `eval $(minikube docker-env)` 设置这些环境变量。
...
如果你需要插件使用远程连接而不是默认的本地连接与 Docker 守护进程通信，连接细节可以使用 `docker` 参数提供，如下例所示:
...
如果构建器或运行镜像存储在支持用户身份验证的私有 Docker 注册中心中，则可以使用 `docker.builderRegistry` 参数提供身份验证细节，如下例所示:
...
如果构建器或运行镜像存储在支持令牌身份验证的私有 Docker 注册中心中，则可以使用`docker.builderRegistry` 参数提供令牌值。如下例所示:
